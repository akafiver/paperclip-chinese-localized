...
type IssueChatTranslate = (key: string, params?: Record<string, unknown>) => string;

function defaultIssueChatTranslate(key: string, params?: Record<string, unknown>): string {
  ...
  switch (key) {
    case "ui.issueChatMessages.duration.seconds":
      return `${params?.count} second${params?.count === 1 ? "" : "s"}`;
    case "ui.issueChatMessages.duration.minutes":
      return `${params?.count} minute${params?.count === 1 ? "" : "s"}`;
    case "ui.issueChatMessages.duration.hours":
      return `${params?.count} hour${params?.count === 1 ? "" : "s"}`;
    case "ui.issueChatMessages.duration.hoursMinutes":
      return `${params?.hours} hour${params?.hours === 1 ? "" : "s"} ${params?.minutes} minute${params?.minutes === 1 ? "" : "s"}`;
    case "ui.issueChatMessages.workedFor":
      return `Worked for ${duration}`;
    case "ui.issueChatMessages.finishedWork":
      return "Finished work";
    case "ui.issueChatMessages.failedAfter":
      return `Failed after ${duration}`;
    case "ui.issueChatMessages.runFailed":
      return "Run failed";
    case "ui.issueChatMessages.timedOutAfter":
      return `Timed out after ${duration}`;
    case "ui.issueChatMessages.runTimedOut":
      return "Run timed out";
    case "ui.issueChatMessages.interruptedAfter":
      return `Interrupted by board after ${duration}`;
    case "ui.issueChatMessages.interrupted":
      return "Interrupted by board";
    case "ui.issueChatMessages.pausedAfter":
      return `Paused by board after ${duration}`;
    case "ui.issueChatMessages.paused":
      return "Paused by board";
    case "ui.issueChatMessages.cancelledAfter":
      return `Cancelled after ${duration}`;
    case "ui.issueChatMessages.runCancelled":
      return "Run cancelled";
    case "ui.issueChatMessages.queued":
      return "Queued";
    case "ui.issueChatMessages.working":
      return "Working...";
    case "ui.issueChatMessages.runFinished":
      return "Run finished";
    case "ui.issueChatMessages.status.succeeded":
      return "succeeded";
    case "ui.issueChatMessages.status.failed":
      return "failed";
    case "ui.issueChatMessages.status.error":
      return "error";
    case "ui.issueChatMessages.status.timedOut":
      return "timed out";
    case "ui.issueChatMessages.status.cancelled":
      return "cancelled";
    case "ui.issueChatMessages.status.queued":
      return "queued";
    case "ui.issueChatMessages.status.running":
      return "running";
    case "ui.issueChatMessages.historicalRun":
      return `${agentName} run ${runId} ${status}`;
    default:
      return key;
  }
}

function formatStatusLabel(status: string, translate: IssueChatTranslate = defaultIssueChatTranslate) {
  switch (status) {
    case "succeeded":
      return translate("ui.issueChatMessages.status.succeeded");
    case "failed":
      return translate("ui.issueChatMessages.status.failed");
    case "error":
      return translate("ui.issueChatMessages.status.error");
    case "timed_out":
      return translate("ui.issueChatMessages.status.timedOut");
    case "cancelled":
      return translate("ui.issueChatMessages.status.cancelled");
    case "queued":
      return translate("ui.issueChatMessages.status.queued");
    case "running":
      return translate("ui.issueChatMessages.status.running");
    default:
...
export function formatDurationWords(ms: number | null, translate: IssueChatTranslate = defaultIssueChatTranslate) {
  if (ms === null || !Number.isFinite(ms) || ms <= 0) return null;
  const totalSeconds = Math.max(1, Math.round(ms / 1000));
  if (totalSeconds < 60) {
    return translate("ui.issueChatMessages.duration.seconds", { count: totalSeconds });
  }
  const totalMinutes = Math.round(totalSeconds / 60);
  if (totalMinutes < 60) {
    return translate("ui.issueChatMessages.duration.minutes", { count: totalMinutes });
  }
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;
  if (minutes === 0) {
    return translate("ui.issueChatMessages.duration.hours", { count: hours });
  }
  return translate("ui.issueChatMessages.duration.hoursMinutes", { hours, minutes });
}
...
function runDurationLabel(run: {
  ...
  errorCode?: string | null;
  resultJson?: Record<string, unknown> | null;
}, translate: IssueChatTranslate = defaultIssueChatTranslate) {
  const start = run.startedAt ?? run.createdAt;
  const end = run.finishedAt ?? null;
  const durationMs = end ? Math.max(0, toTimestamp(end) - toTimestamp(start)) : null;
  const durationText = formatDurationWords(durationMs, translate);
  const stopReason = typeof run.resultJson?.stopReason === "string" ? run.resultJson.stopReason : null;
  switch (run.status) {
    case "succeeded":
      return durationText ? translate("ui.issueChatMessages.workedFor", { duration: durationText }) : translate("ui.issueChatMessages.finishedWork");
    case "failed":
    case "error":
      return durationText ? translate("ui.issueChatMessages.failedAfter", { duration: durationText }) : translate("ui.issueChatMessages.runFailed");
    case "timed_out":
      return durationText ? translate("ui.issueChatMessages.timedOutAfter", { duration: durationText }) : translate("ui.issueChatMessages.runTimedOut");
    case "cancelled":
      if (isOperatorInterruptedRun(run.resultJson, run.errorCode)) {
        return durationText ? translate("ui.issueChatMessages.interruptedAfter", { duration: durationText }) : translate("ui.issueChatMessages.interrupted");
      }
      if (stopReason === "paused") {
        return durationText ? translate("ui.issueChatMessages.pausedAfter", { duration: durationText }) : translate("ui.issueChatMessages.paused");
      }
      return durationText ? translate("ui.issueChatMessages.cancelledAfter", { duration: durationText }) : translate("ui.issueChatMessages.runCancelled");
    case "queued":
      return translate("ui.issueChatMessages.queued");
    case "running":
      return translate("ui.issueChatMessages.working");
    default:
      return formatStatusLabel(run.status, translate);
  }
}

function createHistoricalRunMessage(run: IssueChatLinkedRun, agentMap?: Map<string, Agent>, translate: IssueChatTranslate = defaultIssueChatTranslate) {
  const agentName = run.agentName ?? agentMap?.get(run.agentId)?.name ?? run.agentId.slice(0, 8);
  const message: ThreadSystemMessage = {
    id: `run:${run.runId}`,
    role: "system",
    createdAt: toDate(runTimestamp(run)),
    content: [{ type: "text", text: translate("ui.issueChatMessages.historicalRun", { agentName, runId: run.runId.slice(0, 8), status: formatStatusLabel(run.status, translate) }) }],
    metadata: {
      custom: {
        kind: "run",
...
function createHistoricalTranscriptMessage(args: {
  ...
  hasOutput: boolean;
  agentMap?: Map<string, Agent>;
  translate?: IssueChatTranslate;
}) {
  const { run, transcript, hasOutput, agentMap, translate = defaultIssueChatTranslate } = args;
  const agentName = run.agentName ?? agentMap?.get(run.agentId)?.name ?? run.agentId.slice(0, 8);
  ...
  const waitingText = hasOutput ? "" : translate("ui.issueChatMessages.runFinished");
  ...
  const message: ThreadAssistantMessage = {
    ...
    metadata: createAssistantMetadata({
      ...
      notices,
      waitingText,
      chainOfThoughtLabel: runDurationLabel(run, translate),
...
function createLiveRunMessage(args: {
  ...
  const { run, transcript, translate = defaultIssueChatTranslate } = args;
  const compactedTranscript = compactIssueChatTranscript(transcript);
  const { parts, notices, segments } = buildAssistantPartsFromTranscript(compactedTranscript);
  const waitingText =
    run.status === "queued"
      ? translate("ui.issueChatMessages.queued")
      : parts.length > 0
        ? ""
        : translate("ui.issueChatMessages.working");
...
export function buildIssueChatMessages(args: {
  ...
  const {
    ...
    currentUserId,
    userLabelMap,
    translate = defaultIssueChatTranslate,
  } = args;
  ...
  for (const run of [...linkedRuns].sort((a, b) => toTimestamp(runTimestamp(a)) - toTimestamp(runTimestamp(b)))) {
    ...
    orderedMessages.push({
      createdAtMs: toTimestamp(runTimestamp(run)),
      order: 2,
      message: createHistoricalRunMessage(run, agentMap, translate),
    });
  }
