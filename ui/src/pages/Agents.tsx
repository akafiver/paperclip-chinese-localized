...
import {
  AgentActionButtons,
  TerminatedAgentActions,
} from "../components/AgentActionButtons";
...
const AGENT_FILTER_TAB_VALUES: FilterTab[] = [
  "all",
  "active",
  "paused",
  "error",
...
function getSandboxProviderLabel(
  ...
  const displayName =
    capabilities?.sandboxProviders?.[provider]?.displayName ?? provider;
...
export function Agents() {
  ...
  const effectiveView: "list" | "org" =
    forceListView || tab === "terminated" ? "list" : view;
  const visibleTabItems = useMemo(
    () =>
      AGENT_FILTER_TAB_VALUES.filter(
        (value) => value !== "builtin" || builtInAgentsEnabled,
      ).map((value) => ({
        value,
        label: t(`ui.agents.${value === "builtin" ? "builtIn" : value}`),
      })),
  ...
  const {
    ...
    queryFn: () =>
      agentsApi.list(selectedCompanyId!, { includeTerminated: true }),
    enabled: !!selectedCompanyId,
  });
        ...
        leading={
          hasInvalidOrgChain ? (
            <AlertTriangle
              ...
              aria-label={t("ui.agents.invalidReportingChain")}
            />
          ) : (
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {!forceListView && tab !== "terminated" && (
            <div
              ...
              role="group"
              aria-label={t("ui.agents.viewMode")}
            >
              <button
                className={cn(
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">
          {filtered.length === 1
            ? t("ui.agents.agentCount", { count: filtered.length })
            : t("ui.agents.agentsCount", { count: filtered.length })}
        </p>
      )}
      ...
      >
        {hasInvalidOrgChain ? (
          <AlertTriangle
            ...
            aria-label={translate("ui.agents.invalidReportingChain")}
          />
        ) : (
