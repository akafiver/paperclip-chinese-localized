...
function stageSectionLabel(section: StageSectionKey, t: TFunction) {
  return t(`ui.pipelines.stageSections.${section}`);
}

function stageKindLabel(kind: EditableStageKind, t: TFunction) {
  return t(`ui.pipelines.stageKinds.${kind}.label`);
}

function stageKindDescription(kind: EditableStageKind, t: TFunction) {
  return t(`ui.pipelines.stageKinds.${kind}.description`);
}

function executionWorkspaceOptionLabel(
  value: typeof STAGE_EXECUTION_WORKSPACE_OPTIONS[number]["value"],
  t: TFunction,
) {
  return t(`ui.pipelines.executionWorkspaceOptions.${value}`);
}
...
function fieldOriginLabel(depth: number, pipelineName: string, t: TFunction) {
  if (depth === 0) return t("ui.pipelines.fieldOrigins.thisItem");
  if (depth === 1) return t("ui.pipelines.fieldOrigins.parent", { name: pipelineName });
  if (depth === 2) return t("ui.pipelines.fieldOrigins.grandparent", { name: pipelineName });
  return t("ui.pipelines.fieldOrigins.ancestor", { depth, name: pipelineName });
}

function pipelineCarryOverFields(
  source: { pipeline: PipelineWithOptionalConnections; depth: number },
  t: TFunction,
): CarryOverFieldOption[] {
  ...
  return stages.flatMap((stage) => {
    ...
    return variables.flatMap((variable) => {
      ...
      return [{
        ...
        originLabel: fieldOriginLabel(source.depth, source.pipeline.name, t),
...
function buildCarryOverFieldGroups(
  ...
  return sources.flatMap((source) => {
    const fields = pipelineCarryOverFields(source, t).filter((field) => {
    ...
    return [{
      ...
      label: fieldOriginLabel(source.depth, source.pipeline.name, t),
...
function buildIncomingCarryOverFieldGroups(
  ...
  return [...byId.values()].flatMap((sourcePipeline) => {
    return (sourcePipeline.stages ?? []).flatMap((sourceStage) => {
      ...
      const sourceFieldGroups = buildCarryOverFieldGroups(sourcePipeline, [...byId.values()], t);
...
function automationVariableKindLabel(value: unknown, t: TFunction) {
  if (Array.isArray(value)) return t("ui.pipelines.automationVariableKinds.array");
  if (value == null) return t("ui.pipelines.automationVariableKinds.empty");
  return t(`ui.pipelines.automationVariableKinds.${typeof value}`);
}

function automationVariablePreviewTitle(variable: AutomationVariableOption, t: TFunction) {
  const preview = primitiveAutomationVariablePreview(variable.example);
  const size = typeof variable.example === "string"
    ? t("ui.pipelines.chars", { count: variable.example.length })
    : t("ui.pipelines.chars", { count: preview.length });
  const lines = [
    variable.description,
    t("ui.pipelines.examplePreview", { kind: automationVariableKindLabel(variable.example, t), size }),
  ];
  if (variable.exampleSource) lines.push(t("ui.pipelines.fromSource", { source: variable.exampleSource }));
...
function buildAutomationVariableGroups(input: {
  ...
  const pipelineVariables: AutomationVariableOption[] = [
    {
      key: "pipeline_id",
      label: input.t("ui.pipelines.automationVariables.pipelineId.label"),
      description: input.t("ui.pipelines.automationVariables.pipelineId.description"),
      example: input.pipeline.id,
      exampleSource: null,
    },
    {
      key: "pipeline_key",
      label: input.t("ui.pipelines.automationVariables.pipelineKey.label"),
      description: input.t("ui.pipelines.automationVariables.pipelineKey.description"),
      example: input.pipeline.key,
      exampleSource: null,
    },
    {
      key: "pipeline_name",
      label: input.t("ui.pipelines.automationVariables.pipelineName.label"),
      description: input.t("ui.pipelines.automationVariables.pipelineName.description"),
      example: input.pipeline.name,
      exampleSource: null,
    },
    {
      key: "stage_id",
      label: input.t("ui.pipelines.automationVariables.stageId.label"),
      description: input.t("ui.pipelines.automationVariables.stageId.description"),
      example: input.stage.id,
      exampleSource: null,
    },
    {
      key: "stage_key",
      label: input.t("ui.pipelines.automationVariables.stageKey.label"),
      description: input.t("ui.pipelines.automationVariables.stageKey.description"),
      example: input.stage.key,
      exampleSource: null,
    },
    {
      key: "stage_name",
      label: input.t("ui.pipelines.automationVariables.stageName.label"),
      description: input.t("ui.pipelines.automationVariables.stageName.description"),
  ...
  const itemVariables: AutomationVariableOption[] = [
    {
      key: "title",
      label: input.t("ui.pipelines.automationVariables.itemTitle.label"),
      description: input.t("ui.pipelines.automationVariables.itemTitle.description"),
      example: sampleCase?.title ?? "",
      exampleSource,
    },
    {
      key: "body",
      label: input.t("ui.pipelines.automationVariables.itemBody.label"),
      description: input.t("ui.pipelines.automationVariables.itemBody.description"),
      example: sampleCase?.summary ?? "",
      exampleSource,
    },
    {
      key: "case_id",
      label: input.t("ui.pipelines.automationVariables.itemId.label"),
      description: input.t("ui.pipelines.automationVariables.itemId.description"),
      example: sampleCase?.id ?? "",
      exampleSource,
    },
    {
      key: "case_key",
      label: input.t("ui.pipelines.automationVariables.itemKey.label"),
      description: input.t("ui.pipelines.automationVariables.itemKey.description"),
      example: sampleCase?.caseKey ?? "",
      exampleSource,
    },
    {
      key: "case_title",
      label: input.t("ui.pipelines.automationVariables.itemTitleAlias.label"),
      description: input.t("ui.pipelines.automationVariables.itemTitleAlias.description"),
      example: sampleCase?.title ?? "",
      exampleSource,
    },
    {
      key: "case_version",
      label: input.t("ui.pipelines.automationVariables.itemVersion.label"),
      description: input.t("ui.pipelines.automationVariables.itemVersion.description"),
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    fieldVariables.push({
      ...
      description: input.t("ui.pipelines.automationVariables.itemField.description"),
      example: value,
      exampleSource,
    });
  }
  const groups: AutomationVariableGroup[] = [
    { id: "pipeline", label: input.t("ui.pipelines.automationVariableGroups.pipeline"), variables: pipelineVariables },
    { id: "item", label: input.t("ui.pipelines.automationVariableGroups.item"), variables: itemVariables },
  ];
  if (fieldVariables.length > 0) {
    groups.push({ id: "fields", label: input.t("ui.pipelines.automationVariableGroups.fields"), variables: fieldVariables });
  }
  return groups;
}
    ...
    <div className="rounded-md border border-dashed border-border bg-muted/25 px-3 py-2">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        <span className="text-xs font-semibold uppercase text-muted-foreground">
          {t("ui.pipelines.alreadyAvailableOnChildItems")}
        </span>
      </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {group.fields.map((field) => (
                <button
                  ...
                  title={t("ui.pipelines.insertVariable", { variable: field.key })}
                  aria-label={t("ui.pipelines.insertVariable", { variable: field.key })}
                >
                  {`{{${field.key}}}`}
                </button>
              ))}
...
function AutomationVariableTokenHelper({
  ...
  return (
    <div className="rounded-md border border-border bg-muted/20 px-3 py-2">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        <span className="text-xs font-semibold uppercase text-muted-foreground">
          {label ?? t("ui.pipelines.availableVariables")}
        </span>
      </div>
      <div className="space-y-2">
        {groups.map((group) => (
            ...
            <div className="flex flex-wrap gap-1.5">
              {group.variables.map((variable) => (
                <button
                  ...
                  className="inline-flex h-7 items-center rounded-md border border-border bg-background px-2 font-mono text-xs text-foreground transition-colors hover:bg-accent"
                  title={automationVariablePreviewTitle(variable, t)}
                  aria-label={t("ui.pipelines.insertVariable", { variable: variable.key })}
                >
                  {`{{${variable.key}}}`}
                </button>
              ))}
  ...
  return (
    <>
      <div className="md:hidden">
        <label className="sr-only" htmlFor="stage-section-picker">{t("ui.pipelines.stageSection")}</label>
        <select
        ...
        >
          {groups.map((group) => (
            <optgroup key={group.label} label={t(`ui.pipelines.stageNavGroups.${group.label}`)}>
              {group.items.map((item) => (
                <option key={item.id} value={item.id}>{stageSectionLabel(item.id, t)}</option>
              ))}
            </optgroup>
          ))}
        </select>
      ...
      <nav
        aria-label={t("ui.pipelines.stageSectionsLabel")}
        ...
        {groups.map((group) => (
          <div key={group.label} className="flex flex-col gap-0.5">
            <p className="px-3 py-2 text-(length:--text-micro) font-semibold uppercase tracking-(--tracking-eyebrow) text-muted-foreground/80">
              {t(`ui.pipelines.stageNavGroups.${group.label}`)}
            </p>
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="truncate">{stageSectionLabel(item.id, t)}</span>
                </button>
              );
            })}
          </div>
...
export function PipelineSettings() {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("ui.pipelines.selectCompanyCreateSecrets"));
  ...
  const incomingCarryOverFieldGroups = useMemo(
    () => buildIncomingCarryOverFieldGroups(pipeline, selectedStage, pipelinesListQuery.data ?? [], t),
  ...
  const automationVariableGroups = useMemo(
    () => pipeline && selectedStage
      ? buildAutomationVariableGroups({ pipeline, stage: selectedStage, sampleRow: sampleCaseRow, t })
      : [],
  ...
  useEffect(() => {
    if (!pipeline) return;
    setBreadcrumbs([
      { label: t("ui.pipelines.pipelines"), href: "/pipelines" },
      { label: pipeline.name, href: `/pipelines/${pipeline.id}` },
      { label: t("ui.pipelines.settings") },
    ]);
  ...
  const saveStage = useMutation({
    mutationFn: async () => {
      ...
      if (
        ...
        throw new Error(t("ui.pipelines.chooseExistingWorkspaceBeforeSavingStage"));
      }
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("ui.pipelines.stageSaved"), tone: "success" });
    },
    onError: async (error) => {
      pushToast({
        title: t("ui.pipelines.failedSaveStage"),
        body: error instanceof Error ? error.message : t("ui.pipelines.couldNotSaveStage"),
        tone: "error",
      });
    },
  });
  ...
  const saveStageEnv = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("ui.pipelines.stageSecretsSaved"), tone: "success" });
    },
    onError: async (error) => {
      pushToast({
        title: t("ui.pipelines.failedSaveSecrets"),
        body: error instanceof ApiError
          ? error.message
          : error instanceof Error
            ? error.message
            : t("ui.pipelines.couldNotSaveStageSecrets"),
        tone: "error",
      });
    },
  });
  ...
  const addStage = useMutation({
    mutationFn: async (afterStage: PipelineStage | null) => {
      ...
      const created = await pipelinesApi.createStage(pipelineId, {
        key: nextStageKey(t("ui.pipelines.newStage"), existingKeys),
        name: t("ui.pipelines.newStage"),
        kind: "working",
    ...
    onSuccess: async (created) => {
      ...
      pushToast({ title: t("ui.pipelines.stageAdded"), tone: "success" });
    },
  });
  ...
  const deleteStage = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("ui.pipelines.stageDeleted"), tone: "success" });
    },
    onError: (error) => {
      pushToast({
        title: t("ui.pipelines.failedDeleteStage"),
        body: error instanceof Error ? error.message : t("ui.pipelines.couldNotDeleteStage"),
        tone: "error",
      });
    },
  });
  ...
  const savePipelineDetails = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("ui.pipelines.pipelineUpdated"), tone: "success" });
    },
  });
  ...
  const saveStrictTransitions = useMutation({
    ...
    onSuccess: async () => {
      await refreshPipeline();
      pushToast({ title: t("ui.pipelines.transitionRulesUpdated"), tone: "success" });
    },
    onError: (error) => {
      setStrictTransitionsEnabled(pipeline?.enforceTransitions ?? false);
      pushToast({
        title: t("ui.pipelines.failedUpdateTransitionRules"),
        body: error instanceof Error ? error.message : t("ui.pipelines.couldNotUpdateTransitionRules"),
        tone: "error",
      });
    },
  });
  ...
  const archivePipeline = useMutation({
    ...
    onSuccess: async (_result, archived) => {
      ...
      if (archived) {
        ...
        await refreshPipeline();
        pushToast({ title: t("ui.pipelines.pipelineRestored"), tone: "success" });
      }
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Hexagon} message={t("ui.pipelines.selectCompanyEditSettings")} />;
  }

  if (!pipelineId) {
    return <EmptyState icon={Hexagon} message={t("ui.pipelines.noPipelineSelected")} />;
  }
  ...
  if (!pipeline) {
    return <EmptyState icon={Hexagon} message={t("ui.pipelines.pipelineNotFound")} />;
  }
  ...
  const breakdownCarryOverFieldGroups = buildCarryOverFieldGroups(pipeline, pipelinesListQuery.data ?? [], t);
  ...
  const transitionTargetsControl = !isReviewStage && !isPipelineTerminalStageKind(stageKind) ? (
    <FieldRow label={t("ui.pipelines.allowedNextSteps")}>
      <div className="space-y-2">
        {otherStages.map((stage) => {
          ...
          return (
            ...
            >
              ...
              {isCancelled ? (
                <span className="text-xs text-muted-foreground">{t("ui.pipelines.alwaysAvailable")}</span>
              ) : null}
            </label>
          );
        })}
    ...
    <div className="rounded-lg border border-border">
      <div className="flex items-start justify-between gap-4 border-b border-border p-4">
        <div className="space-y-1">
          <h3 className="text-sm font-semibold text-foreground">{t("ui.pipelines.breakIntoPieces")}</h3>
          <p className="max-w-md text-sm text-muted-foreground">
            {t("ui.pipelines.breakIntoPiecesDescription")}
          </p>
        </div>
        <ToggleSwitch
          aria-label={t("ui.pipelines.breakIntoPieces")}
      ...
      {breakdownEnabled ? (
        <div className="divide-y divide-border px-4">
          <FieldRow label={t("ui.pipelines.createEachPieceIn")}>
            <div className="space-y-1">
              <div className="flex w-full max-w-sm items-center">
                <select
                  aria-label={t("ui.pipelines.createEachPieceIn")}
                ...
                >
                  <option value="">{t("ui.pipelines.choosePipeline")}</option>
                ...
                {breakdownTargetPipelineId ? (
                  <Link
                    ...
                    aria-label={t("ui.pipelines.openPipeline", { name: breakdownTargetPipeline?.name ?? t("ui.pipelines.selected") })}
                    title={t("ui.pipelines.openPipeline", { name: breakdownTargetPipeline?.name ?? t("ui.pipelines.selected") })}
              ...
              {!breakdownTargetPipelineId ? (
                <p className="text-xs text-muted-foreground">{t("ui.pipelines.pipelineInWorkspace")}</p>
              ) : null}
            </div>
          </FieldRow>
          <FieldRow label={t("ui.pipelines.startingAt")}>
            <div className="space-y-1">
              <select
                aria-label={t("ui.pipelines.startingStageForPiece")}
              ...
              >
                <option value="">{t("ui.pipelines.chooseStage")}</option>
                {breakdownTargetStages.map((stage) => (
                  <option key={stage.id} value={stage.key}>{stage.name}</option>
                ))}
              </select>
              <p className="text-xs text-muted-foreground">{t("ui.pipelines.pieceStartStageHint")}</p>
            </div>
          </FieldRow>
          <FieldRow label={t("ui.pipelines.callEachPiece")}>
            <div className="space-y-1">
              <Input
                aria-label={t("ui.pipelines.callEachPiece")}
                value={breakdownPieceNoun}
                onChange={(event) => setBreakdownPieceNoun(event.target.value)}
                placeholder={t("ui.pipelines.piece")}
                className="h-10 w-full max-w-sm"
              />
              <p className="text-xs text-muted-foreground">
                {t("ui.pipelines.callEachPieceHint", { noun: breakdownPieceNounPlural })}
              </p>
            </div>
          </FieldRow>
          <FieldRow label={t("ui.pipelines.carryOver")}>
            <div className="space-y-2">
              <div className="space-y-1 rounded-md border border-dashed border-border bg-muted/30 px-3 py-2 text-xs">
                <p className="text-muted-foreground">
                  {t("ui.pipelines.carryOverHint")}
                </p>
                {breakdownTargetPipelineId ? (
                  <div className="flex flex-wrap items-center gap-1 text-muted-foreground">
                    <span>{t("ui.pipelines.destinationValidation")}:</span>
                    <span className="font-medium text-foreground">
                      {breakdownTargetPipeline?.name ?? t("ui.pipelines.selectedPipeline")}
                    </span>
                ...
                {breakdownIntakeSettingsHref ? (
                  ...
                  >
                    {t("ui.pipelines.reviewDestinationFields")}
                ...
                {breakdownTargetArchived ? (
                  <p className="flex items-center gap-1 text-amber-700 dark:text-amber-300">
                    ...
                    {t("ui.pipelines.destinationArchived")}
                  </p>
                ) : null}
              </div>
              ...
              {breakdownCarryOverFieldGroups.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  {t("ui.pipelines.noCarryOverFields")}
                </p>
              ) : null}
              <p className="text-xs text-muted-foreground">
                {t("ui.pipelines.uniqueNameTitleFields", { noun: breakdownPieceNoun.trim() || t("ui.pipelines.piece") })}
              </p>
            </div>
          </FieldRow>
          <FieldRow label={t("ui.pipelines.thenMoveCaseTo")}>
            <div className="space-y-1">
              <select
                aria-label={t("ui.pipelines.thenMoveCaseTo")}
              ...
              >
                <option value="">{t("ui.pipelines.stayOnThisStep")}</option>
                {otherStages.map((stage) => (
                  <option key={stage.id} value={stage.key}>{stage.name}</option>
                ))}
              </select>
              <p className="text-xs text-muted-foreground">{t("ui.pipelines.asSoonAsPiecesCreated")}</p>
            </div>
          </FieldRow>
          <FieldRow label={t("ui.pipelines.wait")}>
            <div className="space-y-2">
              <label className="flex items-start gap-2 text-sm">
                ...
                />
                <span className="font-medium text-foreground">
                  {t("ui.pipelines.waitUntilPiecesFinished", { noun: breakdownPieceNounPlural })}
                </span>
              </label>
              <select
                aria-label={t("ui.pipelines.moveWhenPiecesFinish")}
              ...
              {breakdownAdvanceTo ? (
                <p className="text-xs text-muted-foreground">
                  {t("ui.pipelines.noSplitStillMoves", { stage: breakdownCopyNames.advanceToName })}
                </p>
              ) : null}
            </div>
          </FieldRow>
    ...
    <div className="space-y-6">
        ...
        <div className="mb-3 flex items-start justify-between gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button type="button" variant="outline" size="icon" className="h-8 w-8" title={t("ui.pipelines.pipelineActions")}>
            ...
            <DropdownMenuContent align="end">
              {isArchived ? (
                ...
                <DropdownMenuItem variant="destructive" onSelect={() => setArchiveDialogOpen(true)}>
                  <Archive className="h-4 w-4" />
                  {t("ui.pipelines.archivePipeline")}
                </DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        <div className="grid gap-3 md:grid-cols-(--gtc-13) md:items-end">
          <div className="space-y-3">
            <label className="block space-y-1.5 text-sm font-medium">
              <span className="sr-only">{t("ui.pipelines.pipelineName")}</span>
              <Input
                aria-label={t("ui.pipelines.pipelineName")}
                value={pipelineName}
            ...
            <label className="block space-y-1.5 text-sm font-medium">
              <span className="sr-only">{t("ui.pipelines.pipelineDescription")}</span>
              <Textarea
                aria-label={t("ui.pipelines.pipelineDescription")}
                value={pipelineDescription}
                onChange={(event) => setPipelineDescription(event.target.value)}
                rows={2}
                placeholder={t("ui.pipelines.addDescription")}
          ...
          {detailsDirty || savePipelineDetails.isPending ? (
            <Button type="submit" disabled={savePipelineDetails.isPending || !pipelineName.trim()}>
              ...
              {savePipelineDetails.isPending ? t("ui.pipelines.saving") : t("ui.pipelines.saveDetails")}
            </Button>
          ) : null}
        </div>
      ...
      <div className="space-y-6">
          {stages.length === 0 ? (
            <EmptyState
              icon={GitBranch}
              message={t("ui.pipelines.noStagesConfigured")}
              action={t("ui.pipelines.addFirstStage")}
                    ...
                    <div key={stage.id} className="flex items-center gap-2">
                      <div className="flex flex-col items-start gap-1">
                        <button
                          ...
                          aria-label={
                            warningCount > 0
                              ? t("ui.pipelines.stageWarningAria", { name: stage.name, count: warningCount })
                              : stage.name
                          }
                          className={cn(
                        ...
                        >
                          <span className="flex items-start justify-between gap-2">
                            ...
                            {warningCount > 0 ? (
                              <span className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-amber-700 dark:text-amber-300">
                                ...
                                {t("ui.pipelines.warningCount", { count: warningCount })}
                              </span>
                            ) : null}
                          </span>
                          <span className="mt-1 block text-xs text-muted-foreground">{t("ui.pipelines.stepNumber", { number: index + 1 })}</span>
                          {stageNewEntriesDisabled(stage) ? (
                            <span className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-amber-700 dark:text-amber-300">
                              ...
                              {t("ui.pipelines.newEntriesPaused")}
                            </span>
                          ) : null}
                        </button>
                        <Link
                      ...
                      {canInsertAfter ? (
                        <button
                          type="button"
                          aria-label={t("ui.pipelines.insertStageAfter", { name: stage.name })}
                ...
                <div className="min-w-0 flex-1 md:px-8">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <h2 className="text-lg font-semibold text-foreground">
                      {stageSectionLabel(activeStageSection, t)}
                    </h2>
                    {activeStageSection === "instructions" ? (
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          title={newEntriesDisabled ? t("ui.pipelines.resumeNewEntries") : t("ui.pipelines.pauseNewEntries")}
                          aria-label={newEntriesDisabled ? t("ui.pipelines.resumeNewEntries") : t("ui.pipelines.pauseNewEntries")}
                        ...
                        <Button
                          ...
                          title={t("ui.pipelines.deleteStageNamed", { name: selectedStage.name })}
                          aria-label={t("ui.pipelines.deleteStageNamed", { name: selectedStage.name })}
                  ...
                  {activeStageSection === "instructions" ? (
                    <div className="w-full max-w-3xl">
                      <div className="divide-y divide-border border-b border-border">
                        ...
                        <FieldRow label={t("ui.pipelines.stepType")}>
                          <div className="max-w-xl space-y-2">
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button
                                  type="button"
                                  variant="outline"
                                  aria-label={t("ui.pipelines.stepType")}
                                ...
                                >
                                  <span className="flex min-w-0 items-center gap-2">
                                    ...
                                    <span className="truncate">{stageKindLabel(selectedStageKindOption.value, t)}</span>
                                  </span>
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <span className="min-w-0">
                                          <span className="block font-medium text-foreground">{stageKindLabel(option.value, t)}</span>
                                          <span className="mt-0.5 block text-xs leading-5 text-muted-foreground">
                                            {stageKindDescription(option.value, t)}
                                          </span>
                                        </span>
                                      </DropdownMenuRadioItem>
                                    );
                            ...
                            <p className="text-sm leading-6 text-muted-foreground">
                              {stageKindDescription(selectedStageKindOption.value, t)}
                            </p>
                          </div>
                        </FieldRow>

                        {stageKind === "review" ? (
                          <FieldRow label={t("ui.pipelines.approver")}>
                            <InlineEntitySelector
                              value={selectedApproval === "any_human" ? "" : selectedApproval}
                              options={approvalOptions}
                              recentOptionIds={recentAssigneeOptionIds}
                              placeholder={t("ui.pipelines.approver")}
                              noneLabel={t("ui.pipelines.anyHuman")}
                              searchPlaceholder={t("ui.pipelines.searchApprovers")}
                              emptyMessage={t("ui.pipelines.noApproversFound")}
                              onChange={(value) => setSelectedApproval(approverValueFromOption(value))}
                              renderTriggerValue={(option) => {
                                if (!option) return <span className="text-muted-foreground">{t("ui.pipelines.anyHuman")}</span>;
                        ...
                        {stageKind === "review" ? (
                          <FieldRow label={t("ui.pipelines.reviewOutcomes")}>
                            <div className="space-y-2">
                              {([
                                [t("ui.pipelines.approvedItemsMoveTo"), approveTarget, setApproveTarget, t("ui.pipelines.chooseStage")],
                                [t("ui.pipelines.declinedItemsMoveTo"), rejectTarget, setRejectTarget, t("ui.pipelines.chooseStage")],
                                [t("ui.pipelines.itemsNeedingChangesMoveTo"), requestChangesTarget, setRequestChangesTarget, t("ui.pipelines.stayInReview")],
                              ...
                              <div className="grid grid-cols-1 items-center gap-2 sm:grid-cols-(--gtc-42)">
                                <span className="text-sm font-medium">{t("ui.pipelines.askNoteRequestChanges")}</span>
                                <div className="sm:justify-self-start">
                                  <ToggleSwitch checked={requireRequestChangesReason} onCheckedChange={setRequireRequestChangesReason} />
                                </div>
                              </div>
                              <div className="grid grid-cols-1 items-center gap-2 sm:grid-cols-(--gtc-42)">
                                <span className="text-sm font-medium">{t("ui.pipelines.askNoteDeclining")}</span>
                                <div className="sm:justify-self-start">
                            ...
                            {reviewTargetsMissing ? (
                              <p className="mt-2 text-sm text-muted-foreground">
                                {t("ui.pipelines.pickReviewTargetsBeforeSaving")}
                              </p>
                            ) : null}
                          </FieldRow>
                        ) : null}
                    ...
                    <div className="mt-8 w-full max-w-3xl space-y-6">
                      <div className="overflow-x-auto overscroll-x-contain">
                        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                          <span>{t("ui.pipelines.whenItemEnters")}</span>
                          <InlineEntitySelector
                            value={stageAssigneeOptionId(stageAssigneeAgentId)}
                            options={stageAssigneeOptions}
                            recentOptionIds={recentAssigneeOptionIds}
                            placeholder={t("ui.pipelines.pickAgent")}
                            noneLabel={t("ui.pipelines.noAutomation")}
                            searchPlaceholder={t("ui.pipelines.searchAgents")}
                            emptyMessage={t("ui.pipelines.noAgentsFound")}
                            onChange={(value) => setStageAssigneeAgentId(stageAssigneeIdFromOption(value))}
                            renderTriggerValue={(option) => {
                              if (!option) return <span className="text-muted-foreground">{t("ui.pipelines.pickAgent")}</span>;
                          ...
                          />
                          <span>{t("ui.pipelines.runsInstructionsThenMoves")}</span>
                        </div>
                      </div>
                      ...
                      {selectedAutomationAgent ? (
                        <>
                          <div className="divide-y divide-border border-y border-border">
                            <FieldRow label={t("ui.pipelines.projectContext")}>
                              <div className="grid gap-2 sm:grid-cols-(--gtc-43)">
                                <InlineEntitySelector
                                  value={stageProjectId}
                                  options={projectOptions}
                                  recentOptionIds={recentProjectIds}
                                  placeholder={t("ui.pipelines.project")}
                                  noneLabel={t("ui.pipelines.noProject")}
                                  searchPlaceholder={t("ui.pipelines.searchProjects")}
                                  emptyMessage={t("ui.pipelines.noProjectsFound")}
                                  ...
                                  renderTriggerValue={(option) =>
                                    option && selectedAutomationProject ? (
                                      ...
                                      <span className="text-muted-foreground">{t("ui.pipelines.project")}</span>
                                    )
                                  }
                                  renderOption={(option) => {
                                    if (!option.id) return <span className="truncate">{option.label}</span>;
                                ...
                                {selectedAutomationProject ? (
                                  ...
                                  <div className="flex h-10 items-center rounded-md border border-dashed border-border px-3 text-sm text-muted-foreground">
                                    {t("ui.pipelines.projectWorkspace")}
                                  </div>
                                )}
                              </div>
                              {selectedAutomationProject && !selectedAutomationProjectWorkspace ? (
                                <p className="mt-2 text-xs text-muted-foreground">
                                  {t("ui.pipelines.noProjectWorkspaceDefault")}
                                </p>
                              ) : null}
                            </FieldRow>

                            {selectedAutomationProject && selectedProjectSupportsExecutionWorkspace ? (
                              <FieldRow label={t("ui.pipelines.executionWorkspace")}>
                                <div className="grid gap-2 sm:grid-cols-(--gtc-43)">
                                  <select
                                    aria-label={t("ui.pipelines.executionWorkspaceMode")}
                                  ...
                                  >
                                    {STAGE_EXECUTION_WORKSPACE_OPTIONS.map((option) => (
                                      <option key={option.value} value={option.value}>
                                        {executionWorkspaceOptionLabel(option.value, t)}
                                      </option>
                                    ))}
                                  </select>
                                  {stageExecutionWorkspacePreference === "reuse_existing" ? (
                                    <select
                                      aria-label={t("ui.pipelines.existingExecutionWorkspace")}
                                      value={stageExecutionWorkspaceId}
                                      onChange={(event) => handleAutomationExecutionWorkspaceIdChange(event.target.value)}
                                      className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                                    >
                                      <option value="">{t("ui.pipelines.chooseExistingWorkspace")}</option>
                                    ...
                                    <div className="flex h-10 items-center rounded-md border border-dashed border-border px-3 text-sm text-muted-foreground">
                                      {stageExecutionWorkspacePreference === "isolated_workspace"
                                        ? t("ui.pipelines.newWorkspaceWillBeCreated")
                                        : t("ui.pipelines.projectDefaultWorkspace")}
                                    </div>
                                  )}
                                </div>
                                {stageExecutionWorkspacePreference === "reuse_existing" && selectedReusableExecutionWorkspace ? (
                                  <p className="mt-2 text-xs text-muted-foreground">
                                    {t("ui.pipelines.reusingWorkspace", { name: selectedReusableExecutionWorkspace.name, source: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("ui.pipelines.existingWorkspace") })}
                                  </p>
                                ) : null}
                                {!canSaveAutomationWorkspace ? (
                                  <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">
                                    {t("ui.pipelines.chooseWorkspaceBeforeSavingReuse")}
                                  </p>
                                ) : null}
                              </FieldRow>
                            ) : null}
                          ...
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            <span>{t("ui.pipelines.agentRunsStepAutomatically", { name: selectedAutomationAgent.name })}</span>
                          </div>
                          <FieldRow label={t("ui.pipelines.issueTitle")}>
                            <Input
                              ref={issueTitleTemplateInputRef}
                              aria-label={t("ui.pipelines.issueTitleTemplate")}
                          ...
                          <AutomationVariableTokenHelper
                            ...
                            label={t("ui.pipelines.issueTitleVariables")}
                          />
                          {breakdownEnabled ? (
                            <div className="space-y-1">
                              <h3 className="text-sm font-semibold text-foreground">{t("ui.pipelines.whatShouldAgentDecide")}</h3>
                              <p className="text-sm text-muted-foreground">
                                {t("ui.pipelines.writeOnlyJudgment")}
                              </p>
                            </div>
                          ) : null}
                          <div data-testid="stage-instructions-editor">
                            <MarkdownEditor
                              ...
                              placeholder={
                                breakdownEnabled
                                  ? t("ui.pipelines.breakdownInstructionsPlaceholder")
                                  : t("ui.pipelines.instructionsPlaceholder")
                              }
                              bordered={false}
                        ...
                        <EmptyState
                          icon={Pause}
                          message={t("ui.pipelines.nothingRunsAutomatically")}
                        />
                      )}
                      <div className="space-y-3">
                  ...
                  {activeStageSection === "advanced" ? (
                    <div className="w-full max-w-3xl space-y-8">
                      <div className="divide-y divide-border border-b border-border">
                        <div className="py-3">
                          <h3 className="text-sm font-semibold text-foreground">{t("ui.pipelines.transitions")}</h3>
                        </div>
                        <FieldRow label={t("ui.pipelines.strictMode")}>
                          <div className="space-y-1.5">
                            <div className="flex items-center gap-3">
                              <ToggleSwitch
                                aria-label={t("ui.pipelines.strictlyEnforceTransitions")}
                              ...
                              <span className="text-sm font-medium text-foreground">
                                {t("ui.pipelines.strictlyEnforceTransitions")}
                              </span>
                            </div>
                            <p className="max-w-2xl text-sm text-muted-foreground">
                              {strictTransitionsEnabled
                                ? t("ui.pipelines.strictTransitionsOnHint")
                                : t("ui.pipelines.strictTransitionsOffHint")}
                            </p>
                          </div>
                        </FieldRow>
                      ...
                      {isPipelineTerminalStageKind(stageKind) ? null : breakdownEnabled ? (
                        <EmptyState
                          ...
                          message={t("ui.pipelines.advancedChildrenHidden")}
                        />
                      ) : (
                        <div className="divide-y divide-border border-b border-border">
                          <div className="py-3">
                            <h3 className="text-sm font-semibold text-foreground">{t("ui.pipelines.children")}</h3>
                          </div>
                          <FieldRow label={t("ui.pipelines.blockChildren")}>
                            <div className="space-y-1.5">
                              <div className="flex items-center gap-3">
                                <ToggleSwitch
                                ...
                                />
                                <span className="text-sm font-medium text-foreground">
                                  {t("ui.pipelines.blockUntilChildrenTerminal")}
                                </span>
                              </div>
                              <p className="max-w-2xl text-sm text-muted-foreground">
                                {t("ui.pipelines.blockChildrenHint")}
                              </p>
                            </div>
                          </FieldRow>
                          <FieldRow label={t("ui.pipelines.advanceChildren")}>
                            <div className="space-y-3">
                              <div className="flex items-center gap-3">
                                <ToggleSwitch
                                ...
                                />
                                <span className="text-sm font-medium text-foreground">
                                  {t("ui.pipelines.advanceWhenLastChildDone")}
                                </span>
                              </div>
                              <div className="grid grid-cols-1 items-center gap-2 sm:grid-cols-(--gtc-44)">
                                <span className="text-sm font-medium text-muted-foreground">{t("ui.pipelines.moveTo")}</span>
                                <select
                                  aria-label={t("ui.pipelines.moveToStageWhenChildrenFinish")}
                              ...
                              <p className="max-w-2xl text-sm text-muted-foreground">
                                {t("ui.pipelines.advanceChildrenHint")}
                              </p>
                            </div>
                          </FieldRow>
                        </div>
                    ...
                    <div className="w-full space-y-3">
                      {stageEventsQuery.isLoading ? (
                        ...
                        <StageEventsList
                          events={stageEvents}
                          stages={stages}
                          emptyMessage={t("ui.pipelines.noStageActivity")}
                        />
                      )}
                    </div>
                  ) : null}
              ...
              {stageDirty || saveStage.isPending ? (
                <div className="sticky bottom-0 z-10 -mx-6 mt-6 flex items-center justify-between gap-3 border-t border-border bg-background/95 px-6 py-3 backdrop-blur motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2">
                  <span className="text-sm text-muted-foreground">
                    {saveStage.isPending ? t("ui.pipelines.savingChanges") : t("ui.pipelines.unsavedChanges")}
                  </span>
                  <Button
                    type="submit"
                  ...
                  >
                    ...
                    {saveStage.isPending ? t("ui.pipelines.saving") : t("ui.pipelines.saveStage")}
                  </Button>
                </div>
              ) : null}
            </form>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("ui.pipelines.deleteStage")}</DialogTitle>
            <DialogDescription>
              {t("ui.pipelines.deleteStageDescription", { name: selectedStage?.name ?? t("ui.pipelines.thisStage") })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            {stages.length > 1 ? (
              <label className="block space-y-1.5 text-sm font-medium">
                <span>{t("ui.pipelines.moveExistingItemsTo")}</span>
                <select
                  aria-label={t("ui.pipelines.moveExistingItemsTo")}
              ...
              <p className="text-sm text-muted-foreground">
                {t("ui.pipelines.onlyStageDeleteHint")}
              </p>
            )}
            {deleteStage.error ? (
          ...
          <DialogFooter>
            ...
            >
              <Trash2 className="h-4 w-4" />
              {deleteStage.isPending ? t("ui.pipelines.deleting") : t("ui.pipelines.deleteStage")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("ui.pipelines.archivePipeline")}</DialogTitle>
            <DialogDescription>
              {t("ui.pipelines.archivePipelineDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <label className="block space-y-1.5 text-sm font-medium">
              <span>{t("ui.pipelines.typeNameToConfirm", { name: pipeline.name })}</span>
              <Input
                aria-label={t("ui.pipelines.archiveConfirmation")}
          ...
          <DialogFooter>
            ...
            >
              <Archive className="h-4 w-4" />
              {archivePipeline.isPending ? t("ui.pipelines.archiving") : t("ui.pipelines.archivePipeline")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
