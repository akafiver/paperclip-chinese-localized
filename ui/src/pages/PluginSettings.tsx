...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("ui.plugins.company"), href: "/dashboard" },
      { label: t("ui.plugins.settings"), href: "/company/settings" },
      { label: t("ui.plugins.instanceSettings"), href: "/company/settings/instance/general" },
      { label: t("ui.plugins.plugins"), href: "/company/settings/instance/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("ui.plugins.pluginDetails") },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("ui.plugins.loadingPluginDetails")}</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("ui.plugins.noDescription");
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: t("ui.plugins.configuration") },
            { value: "status", label: t("ui.plugins.status") },
          ]}
          value={activeTab}
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t("ui.plugins.about")}</h2>
              <div className="grid gap-8 lg:grid-cols-(--gtc-52)">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("ui.plugins.description")}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("ui.plugins.author")}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{t("ui.plugins.categories")}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("ui.plugins.none")}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("ui.plugins.settings")}</h2>
              </div>
              {hasLocalFolders ? (
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("ui.plugins.configureFromEnvironments")}</p>
                  <p className="mt-1 text-muted-foreground">
                    {t("ui.plugins.environmentDriverHint", { driver: driverLabel || t("ui.plugins.thisPlugin") })}
                  </p>
                  <div className="mt-3">
                    <Link to="/company/settings/instance/environments">
                      <Button variant="outline" size="sm">{t("ui.plugins.openEnvironments")}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
                <p className="text-sm text-muted-foreground">
                  {t("ui.plugins.noSettingsRequired")}
                </p>
              ) : null}
            </section>
          </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("ui.plugins.runtimeDashboard")}
                  </CardTitle>
                  <CardDescription>
                    {t("ui.plugins.runtimeDashboardDescription")}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("ui.plugins.workerProcess")}
                        </h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("ui.plugins.status")}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("ui.plugins.pid")}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("ui.plugins.uptime")}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("ui.plugins.pendingRpcs")}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t("ui.plugins.crashes")}
                                  </span>
                                  <span className="text-xs">
                                    {t("ui.plugins.crashCount", { consecutive: dashboardData.worker.consecutiveCrashes, total: dashboardData.worker.totalCrashes })}
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("ui.plugins.lastCrash")}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("ui.plugins.noWorkerProcess")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("ui.plugins.noJobRuns")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("ui.plugins.recentWebhookDeliveries")}
                        </h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("ui.plugins.noWebhookDeliveries")}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("ui.plugins.lastChecked", { time: new Date(dashboardData.checkedAt).toLocaleTimeString() })}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("ui.plugins.runtimeDiagnosticsUnavailable")}
                    </p>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-1.5">
                      ...
                      {t("ui.plugins.recentLogs")}
                    </CardTitle>
                    <CardDescription>{t("ui.plugins.lastLogEntries", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {t("ui.plugins.healthStatus")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t("ui.plugins.checkingHealth")}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("ui.plugins.overall")}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("ui.plugins.lifecycle")}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t("ui.plugins.healthChecksReady")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("ui.plugins.noSpecialPermissions")}</p>
                  )}
                </CardContent>
              </Card>
            </div>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("ui.plugins.selectCompanyLocalFolders")}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("ui.plugins.localFolders")}</h3>
      </div>
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t("ui.plugins.failedLoadLocalFolders")}
        </div>
      ) : null}
      {isLoading ? (
        <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin" />
          {t("ui.plugins.loadingLocalFolders")}
        </div>
      ) : (
        <div className="space-y-3">
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t("ui.plugins.localFolderSaved")
          : t("ui.plugins.localFolderSavedNeedsAttention"),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("ui.plugins.failedSaveLocalFolder") });
    },
  });
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: t("ui.plugins.localFolderPathRequired") });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("ui.plugins.localFolderAbsolutePath") });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy ? t("ui.plugins.healthy") : t("ui.plugins.needsAttention")}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? t("ui.plugins.readWrite") : t("ui.plugins.readOnly")}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric label={t("ui.plugins.configured")} value={status?.configured ? t("ui.plugins.yes") : t("ui.plugins.no")} ok={!!status?.configured} />
        <FolderStatusMetric label={t("ui.plugins.readable")} value={status?.readable ? t("ui.plugins.yes") : t("ui.plugins.no")} ok={!!status?.readable} />
        <FolderStatusMetric
          label={t("ui.plugins.writable")}
          value={access === "read" ? t("ui.plugins.notRequested") : status?.writable ? t("ui.plugins.yes") : t("ui.plugins.no")}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{t("ui.plugins.configuredPath")}</div>
      ...
      {status?.problems?.length ? (
        <div className="space-y-2 rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          <div className="font-medium">{t("ui.plugins.validationProblems")}</div>
...
function FolderRequirements({
  ...
  return (
    <div className="grid gap-3 text-sm md:grid-cols-2">
      <RequirementList
        title={t("ui.plugins.requiredDirectories")}
        items={requiredDirectories}
        missingItems={missingDirectories}
        missingLabel={t("ui.plugins.missingDirectories")}
        inspectionUnavailable={rootNotInspected}
      />
      <RequirementList
        title={t("ui.plugins.requiredFiles")}
        items={requiredFiles}
        missingItems={missingFiles}
        missingLabel={t("ui.plugins.missingFiles")}
        inspectionUnavailable={rootNotInspected}
      />
    </div>
  );
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          <Badge variant="secondary" className="text-(length:--text-nano)">
            {t("ui.plugins.notInspected")}
          </Badge>
        ) : missingItems.length > 0 ? (
          <Badge variant="destructive" className="text-(length:--text-nano)">
            {t("ui.plugins.missingCount", { count: missingItems.length })}
          </Badge>
        ) : (
          <Badge variant="outline" className="text-(length:--text-nano)">{t("ui.plugins.present")}</Badge>
        )}
      </div>
      {items.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("ui.plugins.noneDeclared")}</p>
      )}
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">{t("ui.plugins.configuredRootNotInspected")}</p>
      ) : missingItems.length > 0 ? (
...
function PluginConfigForm({ pluginId, companyId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: (configJson: Record<string, unknown>) => {
      if (!companyId) throw new Error(t("ui.plugins.selectCompanyBeforeSaveConfig"));
      return pluginsApi.saveConfig(pluginId, companyId, configJson);
    },
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("ui.plugins.configurationSaved") });
      setTestResult(null);
      if (companyId) {
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("ui.plugins.failedSaveConfiguration") });
    },
  });
  ...
  const testMutation = useMutation({
    mutationFn: (configJson: Record<string, unknown>) => {
      if (!companyId) throw new Error(t("ui.plugins.selectCompanyBeforeTestConfig"));
      return pluginsApi.testConfig(pluginId, companyId, configJson);
    },
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("ui.plugins.configurationTestPassed") });
      } else {
        setTestResult({ type: "error", text: result.message || t("ui.plugins.configurationTestFailed") });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("ui.plugins.configurationTestFailed") });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
        <Loader2 className="h-4 w-4 animate-spin" />
        {t("ui.plugins.loadingConfiguration")}
      </div>
    );
  }
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            ...
            t("ui.plugins.saveConfiguration")
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("ui.plugins.testing")}
              </>
            ) : (
              t("ui.plugins.testConfiguration")
            )}
          </Button>
        )}
      </div>
