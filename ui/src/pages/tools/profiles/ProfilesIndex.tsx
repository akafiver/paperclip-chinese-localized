...
export function ProfilesIndex({
  ...
  const duplicate = useMutation({
    ...
    onError: (error: unknown) =>
      pushToast({ title: t("ui.toolsProfiles.duplicateFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const archive = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("ui.toolsProfiles.archivedToast"), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) => pushToast({ title: t("ui.toolsProfiles.archiveFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const restore = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("ui.toolsProfiles.restoredToast"), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) => pushToast({ title: t("ui.toolsProfiles.restoreFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const header = (
    <ToolsPageHeader
      title={t("ui.toolsProfiles.accessProfiles")}
      description={t("ui.toolsProfiles.description")}
      actions={
        <>
          <Button variant="outline" onClick={() => setResolverOpen(true)}>
            <ShieldCheck className="mr-1.5 h-4 w-4" />
            {t("ui.toolsProfiles.checkAgentAccess")}
          </Button>
          <Button onClick={() => navigate(newProfileHref())}>
            <Plus className="mr-1.5 h-4 w-4" />
            {t("ui.toolsProfiles.newProfile")}
          </Button>
        </>
      }
    />
    ...
    <Sheet open={resolverOpen} onOpenChange={setResolverOpen}>
      <SheetContent className="w-full gap-0 p-0 sm:max-w-xl">
        <SheetHeader className="border-b border-border">
          <SheetTitle>{t("ui.toolsProfiles.checkAgentAccess")}</SheetTitle>
          <SheetDescription>
            {t("ui.toolsProfiles.checkAgentAccessDescription")}
          </SheetDescription>
        </SheetHeader>
  ...
  if (profiles.isLoading) {
    return (
      <div className="space-y-5">
        {header}
        <LoadingState label={t("ui.toolsProfiles.loadingProfiles")} />
      </div>
    );
  }
  if (profiles.isError) {
    ...
    <div className="space-y-5">
      ...
      <div className="inline-flex rounded-md border border-border p-0.5">
        {(["active", "archived"] as const).map((key) => (
          ...
          >
            {key === "active" ? t("ui.common.active") : t("ui.common.archived")}
          </button>
        ))}
      </div>
      ...
      {rows.length === 0 ? (
        statusFilter === "archived" ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-5 text-sm text-muted-foreground">
            {t("ui.toolsProfiles.noArchivedProfiles")}
          </div>
        ) : (
          ...
          <table className="w-full table-fixed text-sm">
            ...
            <thead>
              <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
                <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.table.profile")}</th>
                <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.table.allows")}</th>
                <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.table.assignedTo")}</th>
                <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.table.status")}</th>
                <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.table.updated")}</th>
                      ...
                      <span className="inline-flex items-center gap-2">
                        ...
                        {(profile.newToolsPendingCount ?? 0) > 0 ? (
                          <Badge variant="outline" className="border-amber-500/50 bg-amber-500/10 text-amber-800 dark:text-amber-200">
                            {t("ui.toolsProfiles.newTools", { count: profile.newToolsPendingCount })}
                          </Badge>
                        ) : null}
                      </span>
                    </td>
                    <td className="px-3 py-1.5">
                      {assigned.unassigned ? (
                        <span className="text-muted-foreground">
                          {assigned.text}
                          <span className="ml-1 text-xs text-muted-foreground/70">- {t("ui.toolsProfiles.noAccessEffect")}</span>
                        </span>
                      ) : (
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          aria-label={t("ui.toolsProfiles.profileActions")}
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onSelect={onEdit}>{t("ui.toolsProfiles.edit")}</DropdownMenuItem>
        <DropdownMenuItem onSelect={onDuplicate}>{t("ui.toolsProfiles.duplicate")}</DropdownMenuItem>
        {onRestore ? (
          ...
          <DropdownMenuItem onSelect={onArchive}>{t("ui.common.archive")}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
  ...
  return (
    <div className="rounded-lg border border-dashed border-border p-6">
      <div className="mb-4 max-w-2xl">
        <h3 className="text-base font-semibold text-foreground">{t("ui.toolsProfiles.createFirstAccessProfile")}</h3>
        <p className="text-sm text-muted-foreground">
          {t("ui.toolsProfiles.pickStartingPoint")}
        </p>
      </div>
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {TEMPLATES.map((template) => (
          ...
          >
            <span className="text-sm font-medium text-foreground">{t(template.titleKey)}</span>
