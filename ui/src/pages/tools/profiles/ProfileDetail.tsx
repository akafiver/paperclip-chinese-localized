...
export function ProfileDetail({
  ...
  const updateProfile = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      pushToast({ title: t("ui.toolsProfiles.detail.profileUpdated"), tone: "success" });
    },
    onError: (error: unknown) => pushToast({ title: t("ui.toolsProfiles.detail.updateFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const duplicateProfile = useMutation({
    ...
    onSuccess: (copy) => {
      invalidate();
      pushToast({
        title: t("ui.toolsProfiles.duplicated"),
        body: t("ui.toolsProfiles.duplicateBody"),
        tone: "success",
      });
      navigate(`/apps/advanced/profiles/${copy.id}?created=1`);
    },
    onError: (error: unknown) => pushToast({ title: t("ui.toolsProfiles.duplicateFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const deleteProfile = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      pushToast({ title: t("ui.toolsProfiles.deletedToast"), tone: "success" });
      navigate("/apps/advanced/profiles");
    },
    onError: (error: unknown) => pushToast({ title: t("ui.toolsProfiles.deleteFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const removeAssignment = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidate();
      pushToast({ title: t("ui.toolsProfiles.detail.assignmentRemoved"), tone: "success" });
    },
    onError: (error: unknown) => pushToast({ title: t("ui.toolsProfiles.detail.removeAssignmentFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const reviewNewTools = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("ui.toolsProfiles.detail.newToolsReviewed"), tone: "success" });
    },
    onError: (error: unknown) => pushToast({ title: t("ui.toolsProfiles.detail.submitReviewFailed"), body: errorBody(error), tone: "error" }),
  });

  if (data.profiles.isLoading) return <LoadingState label={t("ui.toolsProfiles.detail.loadingProfile")} />;
  if (data.profiles.isError) return <ErrorState error={data.profiles.error} onRetry={() => data.profiles.refetch()} />;
  if (!profile) {
    return (
      <div className="space-y-4">
        <ToolsPageHeader title={t("ui.toolsProfiles.detail.notFound")} description={t("ui.toolsProfiles.detail.deletedDescription")} />
        <Button variant="outline" onClick={() => navigate("/apps/advanced/profiles")}>{t("ui.toolsProfiles.detail.backToProfiles")}</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm">
        ...
        <span className="text-muted-foreground">{t("ui.toolsProfiles.detail.updated")} <RelativeTime value={profile.updatedAt} /></span>
      ...
      {created ? (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-primary/30 bg-primary/5 px-4 py-3">
          <div>
            <p className="text-sm font-medium text-foreground">{t("ui.toolsProfiles.detail.profileSaved")}</p>
            <p className="text-sm text-muted-foreground">
              {unassigned ? t("ui.toolsProfiles.detail.assignBeforeAccess") : t("ui.toolsProfiles.detail.assignmentsActive")}
            </p>
          </div>
          <div className="flex gap-2">
            <Button size="sm" onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=3`)}>
              {t("ui.toolsProfiles.detail.assign")}
            </Button>
            <Button size="sm" variant="ghost" onClick={() => setSearchParams({})}>
              {t("ui.toolsProfiles.detail.dismiss")}
            </Button>
          </div>
        </div>
      ) : null}
      ...
      {archived ? (
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
          {t("ui.toolsProfiles.detail.archivedNotice")}
        </div>
      ) : null}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{t("ui.toolsProfiles.detail.whatItAllows")}</h2>
          <Button variant="outline" size="sm" disabled={archived} onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=2`)}>
            {t("ui.toolsProfiles.detail.editTools")}
          </Button>
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{t("ui.toolsProfiles.detail.whoHasIt")}</h2>
      ...
      <section className="space-y-3">
        <h2 className="text-base font-semibold text-foreground">{t("ui.toolsProfiles.detail.laterTools")}</h2>
        <NewToolsSetting
      ...
      <Button variant="link" className="h-auto px-0" onClick={() => navigate("/apps/advanced/profiles?check=1")}>
        ...
        {t("ui.toolsProfiles.detail.checkAgentAccess")}
      </Button>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
      <div>
        <p className="font-medium">
          {loading
            ? t("ui.toolsProfiles.detail.newToolsNeedReview")
            : t(count === 1 ? "ui.toolsProfiles.detail.appAddedTools" : "ui.toolsProfiles.detail.appAddedToolsPlural", {
                app: appLabel,
                count,
              })}
        </p>
        <p className="text-amber-900/80">{t("ui.toolsProfiles.chooseNewTools")}</p>
      </div>
      <Button size="sm" onClick={onReview}>{t("ui.toolsProfiles.detail.review")}</Button>
    </div>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("ui.toolsProfiles.detail.reviewNewTools")}</DialogTitle>
          <DialogDescription>
            {t("ui.toolsProfiles.detail.reviewDescription")}
          </DialogDescription>
        </DialogHeader>
        {loading ? (
          <LoadingState label={t("ui.toolsProfiles.detail.loadingNewTools")} />
        ) : error ? (
          <ErrorState error={error} onRetry={onRetry} />
        ) : tools.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-6 text-sm text-muted-foreground">
            {t("ui.toolsProfiles.detail.noNewTools")}
          </div>
        ) : (
              ...
              <div key={tool.catalogEntryId} className="grid gap-3 px-3 py-3 sm:grid-cols-[1fr_auto]">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {tool.applicationName ?? tool.connectionName ?? t("ui.toolsProfiles.detail.appTool")} · {t("ui.toolsProfiles.detail.added", { date: formatShortDate(tool.addedAt) })}
                  </p>
                </div>
                <div className="flex items-center gap-2 sm:justify-end">
                  <label className="inline-flex items-center gap-1.5 text-sm">
                    ...
                    />
                    {t("ui.toolsProfiles.detail.allow")}
                  </label>
                  <label className="inline-flex items-center gap-1.5 text-sm">
                    <input
                      type="radio"
                    ...
                    />
                    {t("ui.toolsProfiles.detail.keepBlocked")}
                  </label>
                </div>
              </div>
            ))}
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>{t("ui.common.cancel")}</Button>
          <Button disabled={pending || loading || tools.length === 0} onClick={onSubmit}>
            {t("ui.toolsProfiles.detail.submitReview")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function AllowList({ rows, total }: { rows: AllowRow[]; total: number }) {
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
        {t("ui.toolsProfiles.detail.zeroTools")}
      </div>
    );
  }
  return (
    <div className="overflow-hidden rounded-lg border border-border">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
            <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.detail.tool")}</th>
            <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.detail.app")}</th>
            <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.detail.capabilities")}</th>
            <th className="px-3 py-2 font-medium">{t("ui.toolsProfiles.detail.source")}</th>
          </tr>
        </thead>
        <tbody>
          {rows.slice(0, 80).map((row) => (
            <tr key={row.id} className={cn("border-b border-border last:border-0", row.degraded && "bg-muted/30 text-muted-foreground")}>
                ...
                <div className="flex flex-col">
                  ...
                  {row.degraded ? (
                    <a className="mt-0.5 inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline" href={`/apps/${row.connectionId}`}>
                      ...
                      {t("ui.toolsProfiles.detail.reconnect")}
                    </a>
                  ) : null}
                </div>
              </td>
              <td className="px-3 py-2">
                <span>{row.app}</span>
                {row.degraded ? <span className="ml-2 text-xs text-muted-foreground">{t("ui.toolsProfiles.detail.appDisconnected", { app: row.app })}</span> : null}
              </td>
              ...
              <td className="px-3 py-2">
                ...
                {row.autoAddedAt ? (
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    {t("ui.toolsProfiles.detail.addedAutomatically", { date: formatShortDate(row.autoAddedAt) })}
                  </div>
                ) : null}
              </td>
            </tr>
      ...
      {rows.length > 80 ? (
        <p className="border-t border-border px-3 py-2 text-xs text-muted-foreground">
          {t("ui.toolsProfiles.detail.showingAllowed", { count: rows.length })}
        </p>
      ) : (
        <p className="border-t border-border px-3 py-2 text-xs text-muted-foreground">
          {t("ui.toolsProfiles.detail.allowsKnown", { shown: rows.length, total })}
        </p>
      )}
    </div>
  );
  ...
  if (profile.bindings.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-border px-4 py-5">
        <p className="text-sm font-medium text-foreground">{t("ui.toolsProfiles.detail.notAssignedYet")}</p>
        <p className="text-sm text-muted-foreground">{t("ui.toolsProfiles.detail.assignmentHint")}</p>
      </div>
    );
  }
  return (
      ...
      {profile.bindings.map((binding) => (
        <div key={binding.id} className="flex items-center justify-between gap-3 px-3 py-2.5">
          ...
          <Button variant="ghost" size="sm" disabled={archived} onClick={() => onRemove(binding)}>
            ...
            {t("ui.toolsProfiles.detail.remove")}
          </Button>
        </div>
      ))}
    </div>
...
function NewToolsSetting({
  ...
  const options: Array<{ value: ToolProfileDefaultAction; title: string; body: string }> = [
    {
      value: "deny",
      title: t("ui.toolsProfiles.detail.stayBlocked"),
      body: t("ui.toolsProfiles.detail.stayBlockedBody"),
    },
    {
      value: "allow",
      title: t("ui.toolsProfiles.detail.allowAutomatically"),
      body: t("ui.toolsProfiles.detail.allowAutomaticallyBody"),
    },
  ];
  return (
  ...
  if (kind === "edit") {
    return (
      <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("ui.toolsProfiles.detail.editProfile")}</DialogTitle>
            <DialogDescription>{t("ui.toolsProfiles.detail.editDescription")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="edit-profile-name">{t("ui.toolsRunYourOwn.name")}</Label>
              <Input id="edit-profile-name" value={name} onChange={(e) => setName(e.target.value)} />
              {duplicateName ? <p className="text-xs text-destructive">{t("ui.toolsProfiles.detail.nameConflict")}</p> : null}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="edit-profile-description">{t("ui.common.description")}</Label>
            ...
            {advancedOpen ? (
              <div className="space-y-1.5">
                <Label htmlFor="edit-profile-key">{t("ui.toolsProfiles.detail.identifier")}</Label>
  ...
  if (kind === "duplicate") {
    return (
      <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("ui.toolsProfiles.duplicate")}</DialogTitle>
            <DialogDescription>{t("ui.toolsProfiles.detail.duplicateDescription")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="copy-profile-name">{t("ui.toolsRunYourOwn.name")}</Label>
              <Input id="copy-profile-name" value={copyName} onChange={(e) => setCopyName(e.target.value)} />
              {duplicateCopyName ? <p className="text-xs text-destructive">{t("ui.toolsProfiles.detail.nameConflict")}</p> : null}
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={copyAssignments} onChange={(e) => setCopyAssignments(e.target.checked)} />
              {t("ui.toolsProfiles.detail.copyAssignments")}
            </label>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={onClose}>{t("ui.common.cancel")}</Button>
...
function RemoveAssignmentDialog({
  ...
  return (
    <Dialog open={Boolean(binding)} onOpenChange={(next) => !next && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("ui.toolsProfiles.detail.removeAssignment")}</DialogTitle>
          <DialogDescription>
            {binding?.targetType === "company"
              ? t("ui.toolsProfiles.detail.removeCompanyDefaultWarning")
              : t("ui.toolsProfiles.detail.removeProfileFrom", { label })}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>{t("ui.common.cancel")}</Button>
          <Button disabled={pending} onClick={onConfirm}>{t("ui.toolsProfiles.detail.remove")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function buildAllowRows(
  ...
  return catalog
    ...
    .map((tool) => {
      ...
      const app = appNames.get(tool.applicationId ?? "") ?? connectionNames.get(tool.connectionId) ?? translate("ui.toolsProfiles.detail.unknownApp");
      ...
      return {
        ...
        sourceKind: match && ["application", "connection", "risk_level"].includes(match.selectorType) ? "rule" : "direct",
...
function sourceLabel(entry: ToolProfileEntry | null, app: string): string {
  if (!entry) return translate("ui.toolsProfiles.detail.addedDirectly");
  if (entry.selectorType === "application" || entry.selectorType === "connection") {
    return translate("ui.toolsProfiles.detail.addedByAppRule", { app });
  }
  if (entry.selectorType === "risk_level" && entry.riskLevel) {
    return translate("ui.toolsProfiles.detail.addedByRiskRule", { risk: entry.riskLevel });
  }
  return translate("ui.toolsProfiles.detail.addedDirectly");
}

function capabilityLabel(tool: ToolCatalogEntry): string {
  if (tool.isDestructive) return translate("ui.toolsProfiles.detail.destructive");
  if (tool.isWrite) return translate("ui.toolsProfiles.detail.write");
  return translate("ui.toolsProfiles.detail.read");
}

function capabilityText(tool: ToolProfileNewToolReviewItem): string {
  if (tool.riskLevel === "destructive") return translate("ui.toolsProfiles.detail.destructive");
  if (tool.riskLevel === "write") return translate("ui.toolsProfiles.detail.write");
  if (tool.riskLevel === "read") return translate("ui.toolsProfiles.detail.read");
...
function newToolsAppLabel(tools: ToolProfileNewToolReviewItem[]): string {
  ...
  if (names.length === 0) return translate("ui.toolsProfiles.detail.anApp");
  if (names.length === 1) return names[0] ?? translate("ui.toolsProfiles.detail.anApp");
  if (names.length === 2) {
    return translate("ui.toolsProfiles.detail.appAndApp", { first: names[0], second: names[1] });
  }
  return translate("ui.toolsProfiles.detail.appAndMore", { first: names[0], count: names.length - 1 });
}
...
function assignmentLabel(
  ...
  if (binding.targetType === "company") return translate("ui.toolsProfiles.detail.companyDefault");
  if (binding.targetType === "agent") return maps.agentsById.get(binding.targetId) ?? translate("ui.toolsProfiles.detail.unknownAgent");
  if (binding.targetType === "project") return maps.projectsById.get(binding.targetId) ?? translate("ui.toolsProfiles.detail.unknownProject");
  if (binding.targetType === "routine") return maps.routinesById.get(binding.targetId) ?? translate("ui.toolsProfiles.detail.unknownRoutine");
  if (binding.targetId === companyId) return translate("ui.common.company");
  return binding.targetId;
}

function assignmentTypeLabel(type: ToolProfileBinding["targetType"]): string {
  if (type === "company") return translate("ui.toolsProfiles.detail.companyDefault");
  if (type === "agent") return translate("ui.routineRun.agent");
  if (type === "project") return translate("ui.routineRun.project");
  if (type === "routine") return translate("ui.routineRun.runRoutine");
  return translate("ui.toolsProfiles.detail.scopedAssignment");
}
