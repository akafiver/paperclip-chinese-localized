...
export function RunYourOwnTab({ companyId }: { companyId: string }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({
        title: t("ui.toolsRunYourOwn.toolAdded"),
        body: t("ui.toolsRunYourOwn.toolAddedBody", { name: name.trim() }),
        tone: "success",
      });
      setName("");
      setCommand("");
  ...
  return (
    <div className="space-y-6">
      <p className="max-w-2xl text-sm text-muted-foreground">
        {t("ui.toolsRunYourOwn.description")}
      </p>

      <div className="space-y-5 rounded-lg border border-border bg-card p-5">
        <div className="space-y-1.5">
          <Label htmlFor="ryo-name">{t("ui.toolsRunYourOwn.name")}</Label>
          <Input
            id="ryo-name"
            value={name}
            ...
            maxLength={160}
          />
          <p className="text-xs text-muted-foreground">{t("ui.toolsRunYourOwn.nameHint")}</p>
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="ryo-command">{t("ui.toolsRunYourOwn.command")}</Label>
          <Input
            id="ryo-command"
            value={command}
          ...
          />
          <p className="text-xs text-muted-foreground">{t("ui.toolsRunYourOwn.commandHint")}</p>
        </div>

        <div className="space-y-2">
          <div className="flex items-baseline gap-2">
            <Label>{t("ui.toolsRunYourOwn.keys")}</Label>
            <span className="text-xs text-muted-foreground">· {t("ui.toolsRunYourOwn.keysOptional")}</span>
          </div>
          {keyRows.length > 0 ? (
            <div className="space-y-2">
              {keyRows.map((row) => {
                  ...
                  <div key={row.id} className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        aria-label={t("ui.toolsRunYourOwn.removeKey")}
                    ...
                    {invalid ? (
                      <p className="text-xs text-destructive">
                        {t("ui.toolsRunYourOwn.invalidKey")}
                      </p>
                    ) : null}
                  </div>
                );
        ...
        <div className="flex items-start gap-2.5 rounded-md bg-muted/50 px-3 py-2.5">
          ...
          <div className="text-xs">
            <p className="font-medium text-foreground">
              {t("ui.toolsRunYourOwn.isolatedTitle")}
            </p>
            <p className="mt-0.5 flex items-center gap-1 text-muted-foreground">
              <Lock className="h-3 w-3" />
              {t("ui.toolsRunYourOwn.adminOnly")}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button onClick={() => createMutation.mutate()} disabled={!canSubmit || createMutation.isPending}>
            {createMutation.isPending ? t("ui.toolsRunYourOwn.adding") : t("ui.toolsRunYourOwn.checkContinue")}
          </Button>
          <span className="text-xs text-muted-foreground">
            {t("ui.toolsRunYourOwn.registerHint")}
          </span>
        </div>
      </div>

      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground">{t("ui.toolsRunYourOwn.yourOwnTools")}</h3>
        {templates.isLoading ? (
          <LoadingState />
        ) : templates.isError ? (
          <ErrorState error={templates.error} onRetry={() => templates.refetch()} />
        ) : adminTemplates.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("ui.toolsRunYourOwn.empty")}</p>
        ) : (
          <div className="overflow-hidden rounded-lg border border-border">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                  <th className="px-4 py-2.5">{t("ui.toolsRunYourOwn.columns.name")}</th>
                  <th className="px-4 py-2.5">{t("ui.toolsRunYourOwn.columns.command")}</th>
                  <th className="px-4 py-2.5">{t("ui.toolsRunYourOwn.columns.keys")}</th>
                  <th className="px-4 py-2.5">{t("ui.toolsRunYourOwn.columns.added")}</th>
...
function RunYourOwnRow({
  ...
  const disableMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("ui.toolsRunYourOwn.toolOff"), tone: "success" });
      qc.invalidateQueries({ queryKey: queryKeys.tools.stdioTemplates(companyId) });
    },
    onError: (error) => {
      pushToast({
        title: t("ui.toolsRunYourOwn.turnOffFailed"),
  ...
  return (
    <tr className="border-b border-border last:border-0">
      <td className="px-4 py-3">
        ...
        {disabled ? <Badge variant="outline">{t("ui.toolsRunYourOwn.off")}</Badge> : null}
      </td>
      <td className="px-4 py-3">
        <code className="font-mono text-(length:--text-micro) text-muted-foreground">{fullCommand || "—"}</code>
      </td>
      <td className="px-4 py-3 text-muted-foreground">
        {template.envKeys.length > 0 ? template.envKeys.join(", ") : t("ui.toolsRunYourOwn.none")}
      </td>
      <td className="px-4 py-3">
