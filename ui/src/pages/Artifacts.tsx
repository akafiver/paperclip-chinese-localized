...
export function artifactGroupByLabel(value: ArtifactGroupBy): string {
  return ARTIFACT_GROUP_OPTIONS.find((option) => option.value === value)?.value ?? "none";
}
...
export function Artifacts() {
  ...
  useEffect(() => {
    if (viewingSelectedStack && selectedGroup) {
      setBreadcrumbs([
        { label: t("ui.breadcrumb.artifacts"), href: "/artifacts" },
        { label: `${selectedGroup.issue.identifier} · ${selectedGroup.title}` },
      ]);
    } else {
      setBreadcrumbs([{ label: t("ui.breadcrumb.artifacts") }]);
    }
  }, [setBreadcrumbs, viewingSelectedStack, selectedGroup, t]);

  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("ui.selectCompany.artifacts")} />;
  }
  ...
  const emptyMessage = showGroupCards
    ? searching
      ? t("ui.artifacts.noArtifactStacksMatchSearch")
      : t("ui.artifacts.noArtifactStacks")
    : searching
      ? t("ui.artifacts.noArtifactsMatchSearch")
      : viewingSelectedStack
        ? t("ui.artifacts.noArtifactsInStack")
        : kind === "all"
          ? t("ui.artifacts.noArtifactsYet")
          : t("ui.artifacts.noArtifactsOfType");
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input
            ...
            placeholder={t("ui.artifacts.search")}
            aria-label={t("ui.artifacts.search")}
          ...
          {draftQuery.length > 0 ? (
            <button
              ...
              aria-label={t("ui.artifacts.clearSearch")}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                variant="outline"
                size="icon"
                aria-label={t("ui.artifacts.groupByAria", { current: artifactGroupByLabel(groupBy) })}
                title={t("ui.artifacts.groupBy")}
            ...
            <DropdownMenuContent align="end" className="w-44">
              <DropdownMenuLabel>{t("ui.artifacts.groupBy")}</DropdownMenuLabel>
              {ARTIFACT_GROUP_OPTIONS.map((option) => (
                ...
                >
                  {t(`ui.artifacts.${option.value === "parent_task" ? "parentTask" : option.value}`)}
          ...
          <div className="flex flex-wrap items-center gap-1.5" role="tablist" aria-label={t("ui.artifacts.filterByType")}>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div ref={loadMoreRef} className="flex min-h-10 items-center justify-center pb-2 text-xs text-muted-foreground">
            {isFetchingNextPage
              ? t("ui.artifacts.loadingMore")
              : hasNextPage
                ? null
                : isFetching
                  ? t("ui.artifacts.updating")
                  : null}
          </div>
        </>
      )}
