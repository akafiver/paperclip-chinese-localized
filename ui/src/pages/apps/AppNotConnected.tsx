...
export function AppNotConnected() {
  ...
  const appName = application?.name ?? t("ui.appDetail.appFallback");
  useEffect(() => {
    if (!activeTab) return;
    setBreadcrumbs([
  ...
  const remove = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({
        ...
        body: t("ui.appDetail.applicationRemovedBody", { app: appName }),
        tone: "success",
      });
      navigate("/apps");
    },
  ...
  if (!application) {
    return (
      <div className="max-w-3xl space-y-3 p-6 text-sm text-muted-foreground">
        <p>{t("ui.appDetail.missingApplication")}</p>
        <Button variant="outline" size="sm" onClick={() => navigate("/apps")}>{t("ui.appDetail.backToApps")}</Button>
      </div>
    );
  }
  if (activeConnection) {
    ...
    <div className="max-w-3xl space-y-6 pb-12">
      ...
      {activeTab === "review" && (
        previousConnection ? (
          ...
          <EmptyTab
            title={t("ui.appDetail.noReview")}
            body={t("ui.appDetail.reviewAfterConnect")}
          />
        )
      )}
      ...
      {activeTab === "test" && (
        <EmptyTab
          title={t("ui.appDetail.reconnectToTest")}
          body={t("ui.appDetail.testingAfterReconnect")}
        />
      )}
      {activeTab === "activity" && (
        previousConnection ? (
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-bold text-foreground">
              {previousConnection ? t("ui.appDetail.reconnectThisApp") : t("ui.appDetail.connectThisApp")}
            </h2>
            <p className="mt-0.5 text-sm text-muted-foreground">
              {previousConnection
                ? t("ui.appDetail.reconnectPreviousSetup")
                : t("ui.appDetail.agentsCannotUseUntilConnected")}
            </p>
          </div>
          <Button onClick={onConnect}>
            {previousConnection ? t("ui.appDetail.reconnect") : t("ui.appDetail.connect")}
          </Button>
        </div>
      </section>
...
function PreviousSetup({
  ...
  return (
    <section className="rounded-xl border border-border bg-card px-5 py-4">
      <h2 className="text-sm font-bold text-foreground">{t("ui.appDetail.previousSetup")}</h2>
      {connection.healthMessage && (
        <p className="mt-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
          {t("ui.appDetail.lastError", { message: connection.healthMessage })}
        </p>
      )}
      <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-(--gtc-59)">
        ...
        <dt className="text-muted-foreground">{t("ui.appDetail.lastUsed")}</dt>
        <dd className="text-foreground">
          {connection.lastUsedAt ? timeAgo(connection.lastUsedAt) : t("ui.appDetail.never")}
        </dd>
      </dl>
    </section>
  );
...
function PermissionsTab({ previousConnection }: { previousConnection: ToolConnection | null }) {
  ...
  return (
    <section className="rounded-xl border border-border bg-card px-5 py-4">
      <h2 className="text-sm font-bold text-foreground">{t("ui.appDetail.permissionsPaused")}</h2>
      <p className="mt-1 text-sm text-muted-foreground">
        {t("ui.appDetail.permissionsPausedBody")}
      </p>
      {previousConnection && (
        <p className="mt-3 text-xs text-muted-foreground">
          {t("ui.appDetail.previousSetupReadOnly")}
        </p>
      )}
    </section>
  );
  ...
  return (
    <div className="space-y-6">
      {previousConnection ? (
        ...
        <EmptyTab
          title={t("ui.appDetail.noPreviousConnectionDetails")}
          body={t("ui.appDetail.technicalDetailsAfterConnect")}
        />
      )}
