  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="text-sm font-semibold text-foreground">{t("ui.appsBrowse.connectOwnTool")}</div>
        <div className="text-xs text-muted-foreground">
          {t("ui.appsBrowse.connectOwnToolDescription")}
        </div>
      </div>
      <span className="shrink-0 text-xs font-semibold text-primary">{t("ui.appsBrowse.connect")}</span>
    </button>
  );
}
...
export function AdvancedToolsLink() {
  ...
  return (
    ...
    >
      ...
      {t("ui.toolsAccess.developerToolsAdvanced")}
    </Link>
  );
}
