  ...
  async function copyText(value: string, label: string) {
    try {
      if (typeof navigator === "undefined" || !navigator.clipboard?.writeText) {
        throw new Error(t("ui.toolsGateways.clientDialog.clipboardUnavailable"));
      }
      ...
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("ui.toolsGateways.clientDialog.clipboardUnavailable"),
        tone: "error",
      });
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("ui.toolsGateways.clientDialog.title")}</DialogTitle>
          <DialogDescription>
            {t("ui.toolsGateways.clientDialog.description")}
          </DialogDescription>
        </DialogHeader>

        <div className="grid gap-4 sm:grid-cols-(--gtc-10)">
          <nav className="flex gap-1 overflow-x-auto sm:flex-col" aria-label={t("ui.toolsGateways.clientDialog.clients")}>
            ...
            >
              {t("ui.toolsGateways.clientDialog.rawUrl")}
            </button>
          </nav>
          ...
          <div className="min-w-0 space-y-3">
            {active === "raw_url" ? (
              <div className="space-y-1.5">
                <div className="text-sm font-medium text-foreground">{t("ui.toolsGateways.clientDialog.endpointUrl")}</div>
                <div className="flex items-center gap-2">
                  <code className="min-w-0 flex-1 truncate rounded-md bg-muted px-3 py-2 font-mono text-xs text-muted-foreground">
                    {endpoint}
                  </code>
                  <Button variant="outline" size="sm" onClick={() => void copyText(endpoint, t("ui.toolsGateways.clientDialog.endpointUrl"))}>
                    <Copy className="mr-1 h-3.5 w-3.5" />
                    {t("ui.toolsGateways.clientDialog.copy")}
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">
                  {t("ui.toolsGateways.clientDialog.authenticateHint")}
                </p>
              </div>
            ) : activeSnippet ? (
              <div className="space-y-1.5">
              ...
              <p className="text-sm text-muted-foreground">{t("ui.toolsGateways.clientDialog.noSnippets")}</p>
            )}

            <div className="space-y-1.5 rounded-md border border-border p-3">
              <div className="text-xs font-medium text-muted-foreground">{t("ui.toolsGateways.clientDialog.token")}</div>
              {createdToken ? (
                <div className="flex items-center gap-2">
                  ...
                  {revealed ? (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => void copyText(createdToken.token, t("ui.toolsGateways.clientDialog.accessToken"))}
                    >
                      <Copy className="mr-1 h-3.5 w-3.5" />
                      {t("ui.toolsGateways.clientDialog.copy")}
                    </Button>
                    ...
                    <Button variant="outline" size="sm" onClick={() => setRevealed(true)}>
                      {t("ui.toolsGateways.clientDialog.show")}
                    </Button>
                  )}
                </div>
              ) : (
                <p className="text-xs text-muted-foreground">
                  {t("ui.toolsGateways.clientDialog.mintTokenHint")}
                </p>
              )}
              <p className="text-xs text-muted-foreground">
                {t("ui.toolsGateways.clientDialog.passwordHint")}
              </p>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          <Button onClick={() => onOpenChange(false)}>
            ...
            {t("ui.toolsGateways.clientDialog.done")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
