...
export function GatewayDetail() {
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      { label: t("ui.toolsTabs.gateways"), href: "/apps/gateways" },
      { label: gateway.name },
    ]);
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: async (updated) => {
      pushToast({
        title: updated.status === "active" ? t("ui.toolsGateways.gatewayOn") : t("ui.toolsGateways.gatewayOff"),
        body:
          updated.status === "active"
            ? t("ui.toolsGateways.gatewayOnBody", { name: updated.name })
            : t("ui.toolsGateways.gatewayOffBody", { name: updated.name }),
        tone: "success",
      });
  ...
  if (!gateway) {
    return (
      <div className="max-w-3xl p-6">
        <p className="text-sm text-muted-foreground">{t("ui.toolsGateways.notFound")}</p>
        <Button className="mt-4" variant="outline" onClick={() => navigate("/apps/gateways")}>
          {t("ui.toolsGateways.backToGateways")}
        </Button>
      </div>
    );
  }
    ...
    <div className="max-w-4xl space-y-5 pb-12">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-xs text-muted-foreground">
            <Link to="/apps/gateways" className="hover:underline">
              {t("ui.toolsGateways.appsGateways")}
            </Link>
          </div>
        ...
        <Button onClick={() => setSnippetOpen(true)}>
          ...
          {t("ui.toolsGateways.showSnippet")}
        </Button>
      </div>

      <nav className="flex items-center gap-6 overflow-x-auto border-b border-border text-sm" aria-label={t("ui.toolsTabs.gateways")}>
