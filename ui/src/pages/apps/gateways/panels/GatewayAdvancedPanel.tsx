...
export function GatewayAdvancedPanel({
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("ui.toolsGateways.panels.archived"),
        body: t("ui.toolsGateways.panels.archivedBody", { name: gateway.name }),
        tone: "success",
      });
    ...
    onError: (error) =>
      pushToast({
        title: t("ui.toolsGateways.panels.archiveFailed"),
  ...
  return (
    <div className="space-y-5">
      <section className="space-y-2">
        <h3 className="text-sm font-semibold text-foreground">{t("ui.toolsGateways.panels.transport")}</h3>
        <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
          <Row label={t("ui.toolsGateways.panels.transport")} value="streamable_http" />
          <Row label={t("ui.toolsGateways.panels.authentication")} value="bearer" />
          <Row label={t("ui.toolsGateways.panels.protocolVersion")} value="2025-03-26" />
          <Row label={t("ui.toolsGateways.panels.publicId")} value={gateway.gatewayPublicId} mono />
        </dl>
        <div className="flex items-center gap-2">
          <code className="min-w-0 flex-1 truncate rounded-md bg-muted px-3 py-2 font-mono text-xs text-muted-foreground">
            {endpoint}
          </code>
          <Button variant="outline" size="sm" onClick={() => void copy(endpoint, t("ui.toolsGateways.clientDialog.endpointUrl"))}>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-foreground">{t("ui.toolsGateways.panels.rawConfiguration")}</h3>
          <Button variant="outline" size="sm" onClick={() => void copy(rawConfig, t("ui.toolsGateways.panels.gatewayConfigJson"))}>
            <Copy className="mr-1 h-3.5 w-3.5" />
            {t("ui.toolsGateways.panels.copyJson")}
          </Button>
        </div>
      ...
      <section className="space-y-2 rounded-lg border border-destructive/40 p-4">
        <h3 className="text-sm font-semibold text-destructive">{t("ui.toolsGateways.panels.dangerZone")}</h3>
        <p className="text-sm text-muted-foreground">
          {t("ui.toolsGateways.panels.archiveDescription")}
        </p>
        {confirming ? (
          <div className="space-y-2">
            <Input
              ...
              aria-label={t("ui.toolsGateways.panels.typeGatewayName")}
            />
            <div className="flex gap-2">
              ...
              >
                {archiveMutation.isPending ? t("ui.toolsGateways.panels.archiving") : t("ui.toolsGateways.panels.archiveGateway")}
              </Button>
          ...
          <Button variant="outline" size="sm" className="text-destructive" onClick={() => setConfirming(true)}>
            {t("ui.toolsGateways.panels.archiveGateway")}
          </Button>
        )}
      </section>
    </div>
