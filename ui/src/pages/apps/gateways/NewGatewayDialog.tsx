...
export function NewGatewayDialog({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (gateway) => {
      pushToast({ title: t("ui.toolsGateways.created"), body: gateway.name, tone: "success" });
    ...
    onError: (error) => {
      pushToast({
        title: t("ui.toolsGateways.createFailed"),
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{t("ui.toolsGateways.newGateway")}</DialogTitle>
          <DialogDescription>
            {t("ui.toolsGateways.newDescription")}
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={submit}>
          <label className="block space-y-1.5">
            <span className="text-xs font-medium text-muted-foreground">{t("ui.toolsRunYourOwn.name")}</span>
            <Input
              value={name}
          ...
          <label className="block space-y-1.5">
            <span className="text-xs font-medium text-muted-foreground">{t("ui.toolsGateways.accessProfile")}</span>
            <select
            ...
            >
              <option value="" disabled>
                {profilesLoading ? t("ui.toolsGateways.loadingProfiles") : t("ui.toolsGateways.chooseProfile")}
              </option>
            ...
            <span className="text-xs text-muted-foreground">
              {t("ui.toolsGateways.profileDecides")}
            </span>
          </label>
          <label className="block space-y-1.5">
            <span className="text-xs font-medium text-muted-foreground">{t("ui.toolsGateways.descriptionOptional")}</span>
            <textarea
              className="min-h-16 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              value={description}
              onChange={(event) => setDescription(event.target.value)}
              placeholder={t("ui.toolsGateways.descriptionPlaceholder")}
            />
          </label>
          {noProfiles ? (
            <p className="text-xs text-destructive">
              {t("ui.toolsGateways.createProfileFirst")}
            </p>
          ) : null}
          <DialogFooter>
            ...
            >
              {createMutation.isPending ? t("ui.toolsGateways.creating") : t("ui.toolsGateways.createGateway")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
