      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-sm font-bold text-foreground">
            {connected
              ? t("ui.appDetail.connectedWithProvider", { provider: providerName })
              : t("ui.appDetail.connectWithProvider", { provider: providerName })}
          </h2>
          <p className="mt-0.5 text-sm text-muted-foreground">
            {connected
              ? t("ui.appDetail.oauthReconnectDescription")
              : t("ui.appDetail.oauthConnectDescription")}
          </p>
        </div>
        <Button type="button" disabled={disabled} onClick={onStart}>
          {connected ? t("ui.appDetail.reconnect") : t("ui.appDetail.connectWithProvider", { provider: providerName })}
        </Button>
      </div>
    </section>
  );
...
function GoogleSheetsAllowlistSection({
  ...
  return (
    <section className="rounded-xl border border-border bg-card px-5 py-4">
      <div>
        <h2 className="text-sm font-bold text-foreground">{t("ui.appDetail.sheetsAgentsCanUse")}</h2>
        <p className="mt-0.5 text-sm text-muted-foreground">
          {t("ui.appDetail.sheetsAgentsCanUseDescription")}
        </p>
      </div>

      <div className="mt-4 space-y-2">
        {ids.length === 0 ? (
          <div className="text-sm text-muted-foreground">{t("ui.appDetail.noSheetsConnected")}</div>
        ) : (
          ids.map((id) => {
            ...
            return (
              <div key={id} className="flex items-center gap-3 border-t border-border py-2 first:border-t-0">
                ...
                <Button
                  ...
                  title={ids.length <= 1 ? t("ui.appDetail.addAnotherSheetFirst") : undefined}
                  onClick={() => saveIds(ids.filter((current) => current !== id))}
                >
                  {t("ui.appDetail.remove")}
                </Button>
              </div>
            );
          })
        ...
        <Button
          ...
          onClick={() => {
            ...
            if (parsed.ids.length === 0) {
              setError(t("ui.appDetail.pasteSheetsLink"));
              return;
            }
            if (parsed.invalidCount > 0) {
              setError(t("ui.appDetail.invalidSheetsLink"));
              return;
            }
  ...
  return (
    <section className="rounded-xl border border-border bg-card px-5 py-4">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-sm font-bold text-foreground">
            {enabled ? t("ui.appDetail.agentsCanUseThisApp") : t("ui.appDetail.thisAppPaused")}
          </h2>
          <p className="mt-0.5 text-sm text-muted-foreground">
            {enabled
              ? t("ui.appDetail.pauseAppDescription")
              : t("ui.appDetail.resumeAppDescription")}
          </p>
        </div>
        <ToggleSwitch
          aria-label={enabled ? t("ui.appDetail.pauseThisApp") : t("ui.appDetail.resumeThisApp")}
          checked={enabled}
          disabled={disabled}
    ...
    <div className="rounded-xl border border-amber-500/40 bg-amber-500/[0.08] p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm font-semibold text-amber-800 dark:text-amber-200">
          {t("ui.appDetail.newActionsToReview", { count })}
        </div>
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" onClick={() => setOpen((v) => !v)}>
            {open ? t("ui.appDetail.hide") : t("ui.appDetail.review")}
          </Button>
          <Button size="sm" disabled={disabled} onClick={() => onTurnOn(entries.map((e) => e.id))}>
            {t("ui.appDetail.turnOnAll")}
          </Button>
        </div>
      </div>
      <p className="mt-1 text-xs text-amber-700 dark:text-amber-300">
        {t("ui.appDetail.quarantineDescription")}
      </p>
      {open && (
