  ...
  return (
    <section className="rounded-lg border border-border bg-card">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-3 py-2.5">
        <div>
          <h3 className="text-sm font-semibold text-foreground">{t("ui.agentTools.installedApps")}</h3>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t("ui.agentTools.installedAppsDescription", { agent: agentName })}
          </p>
        </div>
                ...
                <div key={connection.id} className="space-y-2 px-3 py-3">
                  <label className="flex items-start gap-3">
                    <Checkbox
                      ...
                      aria-label={t("ui.agentTools.installAppOnAgent", { app: connection.name, agent: agentName })}
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex flex-wrap items-center gap-2">
                        ...
                        {rowPending ? <span className="text-xs text-muted-foreground">{t("ui.agentBubble.saving")}</span> : null}
                      </span>
                      <span className="mt-0.5 block text-xs text-muted-foreground">
                        {installedForAll
                          ? t("ui.agentTools.installedForAllHint")
                          : checked
                            ? t("ui.agentTools.loadedIntoRuntime")
                            : t("ui.agentTools.installedHint")}
                      </span>
                    </span>
                  </label>
                  {permitted && !checked ? (
                    <InlineBanner
                      ...
                      actions={(
                        ...
                        >
                          {t("ui.agentTools.openPermissions")}
                        </Link>
                      )}
                    >
                      {t("ui.agentTools.permittedNotInstalled")}
                    </InlineBanner>
                  ) : null}
                </div>
              );
...
function InstallBadge({
  ...
  const label = installed
    ? (installedForAll ? t("ui.agentTools.installedForAll") : t("ui.agentTools.installed"))
    : permitted
      ? t("ui.appDetail.permittedOnly")
      : t("ui.agentTools.notPermitted");
  return (
    <span
      className={cn(
...
function InstallSaveStatusChip({
  ...
  const { t } = useTranslation();
  if (pending) return <span className="text-xs text-muted-foreground">{t("ui.agentBubble.saving")}</span>;
  if (error) return <span className="text-xs text-destructive">{t("ui.agentTools.couldNotSave")}</span>;
  if (unsaved) return <span className="text-xs text-muted-foreground">{t("ui.agentTools.unsavedChanges")}</span>;
  return <span className="text-xs text-muted-foreground">{t("ui.agentTools.saved")}</span>;
}

const DENIED_TOOLS_DISPLAY_LIMIT = 30;

function policyEffectLabel(policyType: string, t: ReturnType<typeof useTranslation>["t"]): string {
  if (policyType === "allow") return t("ui.agentTools.policyEffects.allow");
  if (policyType === "block") return t("ui.agentTools.policyEffects.block");
  if (policyType === "deny") return t("ui.agentTools.policyEffects.deny");
  if (policyType === "require_approval") return t("ui.agentTools.policyEffects.requireApproval");
  if (policyType === "redact") return t("ui.agentTools.policyEffects.redact");
  if (policyType === "rate_limit") return t("ui.agentTools.policyEffects.rateLimit");
  return policyType;
}
...
export function AgentToolsTab({ agent, companyId }: { agent: AgentDetailRecord; companyId: string }) {
  ...
  if (effective.isLoading) return <ToolsLoadingState label={t("ui.agentTools.resolvingEffectiveAccess")} />;
  if (effective.error) {
  ...
  return (
    <div className="space-y-4">
      <EnforcementBanner
        tone="info"
        title={t("ui.agentTools.effectiveAccess")}
        body={
          <>
            {t("ui.agentTools.effectiveAccessBodyPrefix")}{" "}
            <span className="font-medium">{agent.name}</span>
            {t("ui.agentTools.effectiveAccessBodyMiddle")}{" "}
            <span className="font-medium">{t("ui.agentTools.cannotExpandIt")}</span>
            {t("ui.agentTools.effectiveAccessBodySuffix")}
          </>
        }
      />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <div className="rounded-lg border border-border">
            <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2.5">
              <h3 className="text-sm font-semibold text-foreground">{t("ui.agentTools.allowedTools")}</h3>
              <span className="text-xs text-muted-foreground tabular-nums">
                {t("ui.agentTools.toolsCount", { count: allowedTools.length })}
              </span>
            </div>
            {allowedTools.length === 0 ? (
              <p className="px-3 py-6 text-sm text-muted-foreground">
                {t("ui.agentTools.noAllowedTools")}
              </p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-xs text-muted-foreground">
                    <th className="px-3 py-2 font-medium">{t("ui.agentTools.tool")}</th>
                    <th className="px-3 py-2 font-medium">{t("ui.agentTools.capability")}</th>
                    <th className="px-3 py-2 font-medium">{t("ui.agentTools.risk")}</th>
                    <th className="px-3 py-2 font-medium">{t("ui.agentTools.source")}</th>
                  </tr>
                </thead>
        ...
        <aside className="space-y-4">
          <div className="rounded-lg border border-border bg-background/60 p-3">
                      ...
                      {profile.summary.isCompanyDefault ? (
                        <div className="mt-1">
                          <span className="rounded border border-border px-1.5 py-0.5 text-(length:--text-nano) uppercase text-muted-foreground">
                            {t("ui.toolsProfiles.detail.companyDefault")}
                          </span>
                        </div>
                      ) : null}
                    </div>
            ...
            <div className="mt-3 space-y-1.5">
              <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                {t("ui.agentTools.activePolicies")}
              </div>
              {policiesQuery.isLoading ? (
                <p className="text-xs text-muted-foreground">{t("ui.agentTools.loadingPolicies")}</p>
              ) : governingPolicies.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {t("ui.agentTools.noEnabledPolicy")}
                </p>
              ) : (
            ...
            <div className="mt-3 space-y-1.5">
              <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                {t("ui.agentTools.unavailableTools")}
              </div>
              {catalogLoading ? (
                <p className="text-xs text-muted-foreground">{t("ui.agentTools.checkingTools")}</p>
              ) : deniedTools.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {t("ui.agentTools.everyKnownToolAllowed")}
                </p>
              ) : (
                <>
                  <p className="text-(length:--text-micro) text-muted-foreground">
                    {t("ui.agentTools.blockedToolsDescription")}
                  </p>
                  ...
                  {deniedTools.length > DENIED_TOOLS_DISPLAY_LIMIT ? (
                    <p className="text-(length:--text-micro) text-muted-foreground">
                      {t("ui.agentTools.moreTools", { count: deniedTools.length - DENIED_TOOLS_DISPLAY_LIMIT })}
                    </p>
                  ) : null}
                </>
              )}
