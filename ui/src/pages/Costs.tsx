  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("ui.costs.financeLedger")}</CardTitle>
        <CardDescription>
          {t("ui.costs.financeLedgerDescription")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("ui.costs.debits")}
          value={formatCents(debitCents)}
          subtitle={t("ui.costs.totalEventsInRange", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("ui.costs.credits")}
          value={formatCents(creditCents)}
          subtitle={t("ui.costs.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("ui.costs.net")}
          value={formatCents(netCents)}
          subtitle={t("ui.costs.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("ui.costs.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("ui.costs.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("ui.costs.costs") }]);
  }, [setBreadcrumbs, t]);
    ...
    return [
      {
        ...
        label: (
            <span className="flex items-center gap-1.5">
            <span>{t("ui.costs.allProviders")}</span>
    ...
    return [
      {
        ...
        label: (
            <span className="flex items-center gap-1.5">
            <span>{t("ui.costs.allBillers")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("ui.costs.selectCompany")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("ui.costs.costs")}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("ui.costs.description")}
                </p>
            </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("ui.costs.to")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("ui.costs.inferenceSpend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("ui.costs.inferenceSpendSubtitle", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("ui.costs.budget")}
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("ui.costs.pausedCounts", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? `${formatCents(spendData.summary.spendCents)} of ${formatCents(spendData.summary.budgetCents)}`
                    : t("ui.costs.noMonthlyCap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("ui.costs.financeNet")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("ui.costs.financeNetSubtitle", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("ui.costs.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("ui.costs.financeEventsSubtitle", { amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("ui.costs.overview")}</TabsTrigger>
          <TabsTrigger value="budgets">{t("ui.costs.budgets")}</TabsTrigger>
          <TabsTrigger value="providers">{t("ui.costs.providers")}</TabsTrigger>
          <TabsTrigger value="billers">{t("ui.costs.billers")}</TabsTrigger>
          <TabsTrigger value="finance">{t("ui.costs.finance")}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("ui.costs.selectDateRange")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("ui.costs.inferenceLedger")}</CardTitle>
                    <CardDescription>
                      {t("ui.costs.inferenceLedgerDescription")}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("ui.costs.budgetAmount", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("ui.costs.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("ui.costs.usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("ui.costs.monthlyBudgetConsumed", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("ui.costs.byAgent")}</CardTitle>
                    <CardDescription>{t("ui.costs.byAgentDescription")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("ui.costs.noCostEvents")}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("ui.costs.tokenInOut", { input: formatTokens(row.inputTokens + row.cachedInputTokens), output: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("ui.costs.apiRuns", { count: row.apiRunCount })}
                                    {" · "}
                                    {t("ui.costs.subscriptionRuns", { count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("ui.costs.shortTokens", { tokens: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("ui.costs.byProject")}</CardTitle>
                      <CardDescription>{t("ui.costs.byProjectDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("ui.costs.noProjectCosts")}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("ui.costs.unattributed")}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("ui.costs.noFinanceEventsLong")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-white-card)">
                <CardHeader className="px-5 pt-5 pb-3">
                  ...
                  <CardDescription>
                    {t("ui.costs.budgetControlDescription")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("ui.costs.activeIncidents")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("ui.costs.activeIncidentsSubtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("ui.costs.pendingApprovals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("ui.costs.pendingApprovalsSubtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("ui.costs.pausedAgents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("ui.costs.pausedAgentsSubtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("ui.costs.pausedProjects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("ui.costs.pausedProjectsSubtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("ui.costs.activeIncidents")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("ui.costs.activeIncidentsDescription")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`ui.costs.scopeBudgets.${scopeType}`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("ui.costs.companyBudgetDescription")
                            : scopeType === "agent"
                              ? t("ui.costs.agentBudgetDescription")
                              : t("ui.costs.projectBudgetDescription")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("ui.costs.noCostEventsPeriod")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("ui.costs.noBillableEventsPeriod")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("ui.costs.byBiller")}</CardTitle>
                      <CardDescription>{t("ui.costs.byBillerDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("ui.costs.noFinanceEvents")}</p>
                      ) : (
