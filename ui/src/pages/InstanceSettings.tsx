...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("ui.instanceSettings.settings"), href: "/company/settings" },
      { label: t("ui.instanceSettings.instanceSettings"), href: "/company/settings/instance/general" },
      { label: t("ui.instanceSettings.heartbeats") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("ui.instanceSettings.failedUpdate"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("ui.instanceSettings.unknownError");
        throw new Error(
          failures.length === 1
            ? `${t("ui.instanceSettings.failedDisableAll")} ${detail}`
            : `${t("ui.instanceSettings.failedDisableAll")} ${failures.length}/${enabled.length}: ${detail}`,
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("ui.instanceSettings.failedDisableAll"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("ui.instanceSettings.loading")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("ui.instanceSettings.failedLoad")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("ui.instanceSettings.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("ui.instanceSettings.description")}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("ui.instanceSettings.active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("ui.instanceSettings.disabled")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("ui.instanceSettings.company") : t("ui.instanceSettings.companies")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("ui.instanceSettings.agent") : t("ui.instanceSettings.agents");
              if (!window.confirm(t("ui.instanceSettings.disableConfirm", { count: enabledCount, noun }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("ui.instanceSettings.disabling") : t("ui.instanceSettings.disableAll")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("ui.instanceSettings.noMatch")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("ui.instanceSettings.on") : t("ui.instanceSettings.off")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("ui.instanceSettings.never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("ui.instanceSettings.fullConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("ui.instanceSettings.disableTimer") : t("ui.instanceSettings.enableTimer")}
                          </Button>
                        </span>
                      </div>
                    );
