...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("ui.instanceAccess.instanceSettings"), href: "/company/settings/instance/general" },
      { label: t("ui.instanceAccess.access") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("ui.instanceAccess.companyAccessUpdated"), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("ui.instanceAccess.roleUpdated"), tone: "success" });
    },
  });

  if (usersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("ui.instanceAccess.loadingUsers")}</div>;
  }

  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? t("ui.instanceAccess.adminRequired")
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : t("ui.instanceAccess.failedLoadUsers");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("ui.instanceAccess.title")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("ui.instanceAccess.description")}
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-(--gtc-34)">
        <Card className="block space-y-4 p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{t("ui.instanceAccess.searchUsers")}</span>
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-2"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={t("ui.instanceAccess.searchPlaceholder")}
            />
          </label>
        ...
        <Card className="block space-y-4 p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{t("ui.instanceAccess.selectUser")}</div>
          ) : userAccessQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">{t("ui.instanceAccess.loadingUserAccess")}</div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : t("ui.instanceAccess.failedLoadUserAccess")}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin ? t("ui.instanceAccess.removeAdmin") : t("ui.instanceAccess.promoteAdmin")}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("ui.instanceAccess.companyAccess")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("ui.instanceAccess.companyAccessDesc")}
                  </p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending ? t("ui.instanceAccess.saving") : t("ui.instanceAccess.saveAccess")}
                  </Button>
                </div>
              </div>

              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{t("ui.instanceAccess.currentMemberships")}</h2>
                <div className="space-y-2">
