...
function IssueAssigneePausedNotice({ agent }: { agent: Agent | null }) {
  ...
  const pauseDetail =
    agent.pauseReason === "budget"
      ? translate("ui.issueChat.systemNotice.assigneePausedBudget")
      : agent.pauseReason === "system"
        ? translate("ui.issueChat.systemNotice.assigneePausedSystem")
        : translate("ui.issueChat.systemNotice.assigneePausedManual");
    ...
    <div className="mb-3 rounded-md border border-orange-300/70 bg-orange-50/90 px-3 py-2.5 text-sm text-orange-950 shadow-sm dark:border-orange-500/40 dark:bg-orange-500/10 dark:text-orange-100">
      <div className="flex items-start gap-2">
        ...
        <p className="min-w-0 leading-5">
          {translate("ui.issueChat.systemNotice.assigneePaused", { name: agent.name })} {pauseDetail}
        </p>
      </div>
    </div>
  );
...
const IssueChatComposer = forwardRef<IssueChatComposerHandle, IssueChatComposerProps>(function IssueChatComposer({
  ...
  return (
    <div
      ...
      className={cn(
        "relative rounded-md border border-border/70 bg-background/95 p-(--sz-15px) shadow-(--shadow-overlay) backdrop-blur transition-(--tp-border-color-background-color-box-shadow) duration-150 supports-[backdrop-filter]:bg-background/85 dark:shadow-(--shadow-overlay-dark)",
        pendingWorkModeMeta.classes.container,
        isDragOver && "border-primary/45 bg-background shadow-(--shadow-drag)",
      )}
