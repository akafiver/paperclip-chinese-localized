...
export function SidebarAccountMenu({
  ...
  const secondaryLabel =
    ...
    (deploymentMode === "authenticated" ? t("ui.account.signedIn") : t("ui.account.localWorkspaceBoard"));
  const accountBadge = deploymentMode === "authenticated" ? t("ui.account.account") : t("ui.account.local");
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            aria-label={t("ui.account.openMenu")}
          >
            <Avatar size="sm">
        ...
        >
          <div className="h-24 bg-(image:--gradient-brand)" />
          <div className="-mt-8 px-4 pb-4">
            ...
            <div className="mt-4 space-y-1">
              <MenuAction
                label={t("ui.account.viewProfile")}
                description={t("ui.account.viewProfileDescription")}
                icon={UserRound}
                href={profileHref}
                onClick={closeNavigationChrome}
              />
              <MenuAction
                label={t("ui.account.editProfile")}
                description={t("ui.account.editProfileDescription")}
                icon={UserRoundPen}
                href={PROFILE_SETTINGS_PATH}
                onClick={closeNavigationChrome}
              />
              <MenuAction
                label={t("ui.account.documentation")}
                description={t("ui.account.documentationDescription")}
                icon={BookOpen}
                href={DOCS_URL}
                external
              ...
              />
              <MenuAction
                label={t("ui.account.feedback")}
                description={t("ui.account.feedbackDescription")}
                icon={Megaphone}
                href={FEEDBACK_URL}
                external
                onClick={() => setOpen(false)}
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-sm font-medium text-foreground">
                      {signOutMutation.isPending ? t("ui.account.signingOut") : t("ui.account.signOut")}
                    </span>
                    <span className="block text-xs text-muted-foreground">
                      {t("ui.account.signOutDescription")}
                    </span>
                  </span>
                </button>
              ) : null}
