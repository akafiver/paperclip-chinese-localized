  ...
  override render() {
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-4 px-4 py-10">
        <div>
          <h1 className="text-lg font-semibold">{translate("ui.routeError.title")}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {translate("ui.routeError.description")}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={this.props.onReset}>
            {translate("ui.routeError.goBack")}
          </Button>
          <Button size="sm" onClick={() => window.location.reload()}>
            {translate("ui.routeError.reloadPage")}
          </Button>
        </div>
      </div>
    );
