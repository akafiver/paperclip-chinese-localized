...
export function SkillFolderRail({
  ...
  return (
    <div className="relative hidden h-full shrink-0 md:flex" style={{ width: `${width}px` }}>
      <nav
        aria-label={t("ui.skills.skillFolders")}
      ...
      <div className="mb-2 flex items-center justify-between gap-2 pt-0.5">
        ...
        <Button variant="ghost" size="icon-sm" title={t("ui.skills.newFolder")} onClick={() => onCreateFolder(null)}>
        ...
        <div className="space-y-0.5">
          ...
          {model.my ? (
            <TreeBranch
              ...
              rootLabel={t("ui.skills.mySkills")}
          ...
          <RailHeading label={t("ui.skills.company")} onCreate={() => onCreateFolder(null)} />
          {model.company.length > 0 ? (
            ...
            <div className="px-2 py-1 text-xs text-muted-foreground">{t("ui.skills.noCompanyFoldersYet")}</div>
          )}
          ...
          {model.projects ? (
            <TreeBranch
              ...
              renamingId={renamingId}
              renameDraft={renameDraft}
              rootLabel={t("ui.skills.projects")}
      ...
      <div
        role="separator"
        aria-label={t("ui.skills.resizeSkillFolders")}
  ...
  return (
    <div>
      ...
      >
        <button
          type="button"
          aria-label={isOpen ? t("ui.skills.collapseFolder") : t("ui.skills.expandFolder")}
          className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("ui.skills.folderActionsFor", { name: label })}
              >
            ...
            <DropdownMenuContent align="end">
              {canNest ? (
                <DropdownMenuItem onSelect={() => onCreateFolder(folder.id)}>
                  ...
                  {t("ui.skills.newSubfolder")}
                </DropdownMenuItem>
              ) : null}
              {editable ? (
                <>
                  <DropdownMenuItem onSelect={() => onStartRename(folder)}>{t("ui.skills.rename")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onEditFolder(folder)}>{t("ui.skills.editColor")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onMoveFolder(folder, isInMySkills ? "company" : "my")}>
                    <MoveRight className="h-3.5 w-3.5" />
                    {t("ui.skills.moveToScope", { scope: isInMySkills ? t("ui.skills.company") : t("ui.skills.mySkills") })}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
...
export function FolderBreadcrumb({
  ...
  return (
    <nav aria-label={t("ui.skills.folderPath")} className="flex flex-wrap items-center gap-1 text-sm">
      <button
        type="button"
      ...
      >
        ...
        {t("ui.skills.allSkills")}
      </button>
      {selection === "unfiled" ? (
        <>
          <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/60" />
          <span className="rounded px-1.5 py-0.5 font-medium text-foreground">{t("ui.skills.unfiled")}</span>
        </>
      ) : null}
    ...
    return (
      <div key={folder.id}>
        ...
        >
          ...
          {isCurrent ? <span className="text-xs text-muted-foreground">{t("ui.skills.current")}</span> : null}
          {bundled ? <span className="text-xs text-muted-foreground">{t("ui.skills.readOnly")}</span> : null}
          {isChosen ? <Check className="h-3.5 w-3.5" /> : null}
          {nestable ? (
            <button
              type="button"
              title={t("ui.skills.newFolderInside")}
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="max-h-72 overflow-y-auto rounded-md border border-border p-1">
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate">{t("ui.skills.unfiled")}</span>
            {currentFolderId == null ? <span className="text-xs text-muted-foreground">{t("ui.skills.current")}</span> : null}
        ...
        <div className="min-h-5 text-xs text-muted-foreground">
          {previewPath ? (
            <span>
              {t("ui.skills.movingTo")} <span className="font-mono text-foreground">{previewPath}</span>
            </span>
          ) : (
            <span>{t("ui.skills.pickDestinationFolder")}</span>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          >
            {pending ? t("ui.skills.moving") : t("ui.skills.moveHere")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex items-center gap-2 py-1" style={{ paddingLeft: `${0.5 + depth * 0.9}rem` }}>
      ...
      <Input
        ...
        placeholder={t("ui.skills.folderName")}
        autoFocus
