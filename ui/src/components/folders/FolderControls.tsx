...
function selectionLabel({
  ...
  if (selection === "unfiled") return t("ui.skills.unfiled");
...
export function FolderRail({
  ...
  return (
    <nav aria-label={t("ui.folderControls.foldersForItems", { itemLabelPlural })} className="hidden w-(--sz-folder-rail) shrink-0 border-r border-border pr-3 md:block">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("ui.skills.folders")}</div>
        <Button variant="ghost" size="icon-sm" title={t("ui.skills.newFolder")} onClick={onCreate}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          ...
          {renderVirtualRow("unfiled", t("ui.skills.unfiled"), result?.unfiledCount ?? 0, <FolderSwatch color={null} className="mt-0.5" />)}
        </div>
      )}
    </nav>
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("ui.skills.folderActionsFor", { name: folder.name })}
          >
            <MoreHorizontal className="h-3.5 w-3.5" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end">
          <DropdownMenuItem onSelect={onStartRename}>{t("ui.skills.rename")}</DropdownMenuItem>
          <DropdownMenuItem onSelect={onEdit}>{t("ui.skills.editColor")}</DropdownMenuItem>
          <DropdownMenuSeparator />
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-sm">
      ...
      <span className="min-w-0 flex-1 text-muted-foreground">
        {t("ui.folderControls.groupItemsHint", { itemLabelPlural })}
      </span>
      <Button size="sm" variant="outline" onClick={onCreateFolder}>
        {t("ui.folderControls.createFirstFolder")}
      </Button>
      <Button size="icon-sm" variant="ghost" aria-label={t("ui.folderControls.dismissFolderSuggestion")} onClick={dismiss}>
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-(--sz-folder-sheet-max) rounded-t-lg pb-4">
        <SheetHeader className="border-b border-border px-4 py-3">
          <SheetTitle>{t("ui.folderControls.foldersForItems", { itemLabelPlural })}</SheetTitle>
        </SheetHeader>
        <div className="overflow-y-auto px-3">
          ...
          {result?.kind === "skill" ? (
            <>
              {model.my ? renderBranch(model.my, t("ui.skills.mySkills")) : null}
              ...
              {model.projects ? renderBranch(model.projects, t("ui.skills.projects")) : null}
              {model.bundled ? renderBranch(model.bundled, t("ui.skills.bundled")) : null}
            </>
          ) : (
...
export function MoveToMenu({
  ...
  return (
    <DropdownMenuSub>
      <DropdownMenuSubTrigger>{t("ui.skills.moveTo")}</DropdownMenuSubTrigger>
    ...
    <>
        <div className="flex items-center gap-2 px-2 py-1.5">
          ...
          <input
            ...
            placeholder={t("ui.skills.searchFolders")}
        ...
        {visibleFolders.length === 0 ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">{t("ui.skills.noFoldersMatch")}</div>
        ) : null}
        <DropdownMenuSeparator />
        <DropdownMenuItem onSelect={onCreateAndMove}>
          <Plus className="h-3.5 w-3.5" />
          {t("ui.skills.newFolderEllipsis")}
        </DropdownMenuItem>
    </>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isEdit ? t("ui.skills.editFolder") : t("ui.skills.createFolder")}</DialogTitle>
          <DialogDescription>
            {kind === "routine" ? t("ui.folderControls.organizeRoutines") : t("ui.folderControls.organizeSkills")}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="folder-name">{t("ui.skills.name")}</label>
            <Input
              id="folder-name"
              value={name}
          ...
          <div className="space-y-2">
            <div className="text-sm font-medium">{t("ui.skills.color")}</div>
            <div className="flex flex-wrap gap-2">
              {FOLDER_COLORS.map((swatch) => (
                <button
                  key={swatch}
                  type="button"
                  aria-label={t("ui.folderControls.useFolderColor", { color: swatch })}
                  className={cn(
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onSubmit({ name: name.trim(), color })} disabled={pending || !name.trim()}>
            {pending ? t("ui.skills.saving") : isEdit ? t("ui.skills.save") : t("ui.skills.createFolder")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export function DeleteFolderDialog({
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{t("ui.skills.deleteFolder")}</AlertDialogTitle>
          <AlertDialogDescription>
            {t("ui.folderControls.deleteFolderDescription", { count: folder?.itemCount ?? 0, itemLabelPlural })}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel disabled={pending}>{t("ui.skills.cancel")}</AlertDialogCancel>
          <AlertDialogAction
          ...
          >
            {pending ? t("ui.folderControls.deleting") : t("ui.skills.deleteFolder")}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
export function BulkBar({
  ...
  return (
    <div className="sticky top-2 z-10 flex flex-wrap items-center gap-2 rounded-md border border-border bg-background/95 px-3 py-2 shadow-sm backdrop-blur">
      <span className="mr-auto text-sm text-muted-foreground">{t("ui.folderControls.selectedCount", { count: selectedCount })}</span>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button size="sm" variant="outline">{t("ui.skills.moveTo")}</Button>
        </DropdownMenuTrigger>
      ...
      <Button size="sm" variant="ghost" onClick={onClear}>{t("ui.folderControls.deselectAll")}</Button>
      <Button size="sm" onClick={onDone}>{t("ui.skills.done")}</Button>
    </div>
  );
}
