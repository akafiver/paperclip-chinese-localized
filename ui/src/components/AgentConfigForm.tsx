...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error(t("ui.agentConfig.selectCompanyToTest"));
      }
      const flushedEnv = flushEnvironmentDraft();
      ...
      const primaryModelLabel = t("ui.agentConfig.primaryModel");
      const cheapModelLabel = t("ui.agentConfig.cheapModel");
  ...
  const testActionLabel = t("ui.agentConfig.test");
  ...
  const runEnvironmentTest = useCallback(async () => {
    ...
    try {
      ...
      setTestActionError(error instanceof Error ? error.message : t("ui.agentConfig.environmentTestFailed"));
      throw error;
    } finally {
      ...
      errorMessage: testActionError
        ?? (testEnvironment.error instanceof Error
          ...
          : testEnvironment.error
            ? t("ui.agentConfig.environmentTestFailed")
            : null),
  ...
  async function handleRefreshModels() {
    ...
    try {
      ...
      setRefreshModelsError(error instanceof Error ? error.message : t("ui.agentConfig.failedRefreshModels"));
    } finally {
    ...
    adapterType === "codex_local"
      ...
      : adapterType === "cursor"
        ...
        : adapterType === "opencode_local"
          ? "variant"
          : "effort";
  const thinkingEffortOptions = (adapterType === "codex_local"
      ? codexThinkingEffortOptions
      : adapterType === "cursor"
        ? cursorModeOptions
        : adapterType === "opencode_local"
          ? openCodeThinkingEffortOptions
          : claudeThinkingEffortOptions).map((option) => ({
            id: option.id,
            label: t(`ui.agentConfig.thinkingOptions.${option.labelKey}`),
          }));
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{t("ui.agentConfig.unsaved")}</span>
            <Button
              size="sm"
              onClick={handleSave}
              disabled={!isCreate && props.isSaving}
            >
              {!isCreate && props.isSaving ? t("ui.agentConfig.saving") : t("ui.agentConfig.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("ui.agentConfig.identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("ui.agentConfig.identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("ui.agentConfig.name")} hint={help.name}>
              <DraftInput
                value={eff("identity", "name", props.agent.name)}
                onCommit={(v) => mark("identity", "name", v)}
                immediate
                className={inputClass}
                placeholder={t("ui.newAgent.namePlaceholder")}
              />
            </Field>
            <Field label={t("ui.agentConfig.title")} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("ui.agentConfig.titlePlaceholder")}
              />
            </Field>
            <Field label={t("ui.agentConfig.reportsTo")} hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel={t("ui.agentConfig.chooseManager")}
              />
            </Field>
            <Field label={t("ui.agentConfig.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("ui.agentConfig.capabilitiesPlaceholder")}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("ui.agentConfig.promptTemplate")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("ui.agentConfig.promptPlaceholder")}
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-900 dark:text-amber-100">
                  {t("ui.agentConfig.promptReplayWarning")}
                </div>
              </>
            )}
          </div>
      ...
      {forcedKubernetes ? (
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("ui.agentConfig.environment")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("ui.agentConfig.environment")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("ui.agentConfig.defaultEnvironment")}
              hint={t("ui.agentConfig.forcedKubernetesHint")}
            >
              {kubernetesEnvironment ? (
                <div className={cn(inputClass, "flex items-center text-muted-foreground")}>
                  {kubernetesEnvironment.name} · {t("ui.agentConfig.kubernetesSandbox")}
                </div>
              ) : (
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
                  {t("ui.agentConfig.forcedKubernetesMissing")}
                </div>
              )}
            </Field>
          </div>
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("ui.agentConfig.environmentOverride")}>
              <div className="space-y-2">
                ...
                >
                  <option value="">{t("ui.agentConfig.defaultEnvironmentOption", { label: inheritedEnvironmentLabel })}</option>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("ui.agentConfig.adapter")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("ui.agentConfig.adapter")}</span>
          }
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("ui.agentConfig.adapterType")} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("ui.agentConfig.permissionsConfiguration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("ui.agentConfig.permissionsConfiguration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("ui.agentConfig.command")} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {supportsModelProfiles && (
                <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("ui.agentConfig.primaryModel")}</div>
              )}
              <ModelDropdown
                models={models}
                ...
                detectModelLabel={t("ui.agentConfig.detectModel")}
                emptyDetectHint={t("ui.agentConfig.noModelDetected")}
              />
                ...
                <p className="text-xs text-destructive">
                  {refreshModelsError
                    ?? (fetchedModelsError instanceof Error
                      ...
                      : t("ui.agentConfig.failedLoadAdapterModels"))}
                </p>
              )}
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("ui.agentConfig.openCodeDiscoveryLocalOnly", { environment: currentDefaultEnvironment.name })}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("ui.agentConfig.minimalThinkingSearchWarning")}
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("ui.agentConfig.bootstrapPromptLegacy")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("ui.agentConfig.bootstrapPromptPlaceholder")}
                      contentClassName="min-h-(--sz-44px) text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
                    {t("ui.agentConfig.bootstrapPromptLegacyWarning")}
                  </div>
                </>
              )}
              ...
              <Field label={t("ui.agentConfig.extraArgs")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  className={inputClass}
                  placeholder={t("ui.agentConfig.extraArgsPlaceholder")}
                />
              </Field>

              <Field label={t("ui.agentConfig.environmentVariables")} hint={help.envVars}>
                <EnvironmentVariablesEditor
              ...
              {!isCreate && (
                <>
                  <Field label={t("ui.agentConfig.timeoutSec")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("ui.agentConfig.graceSec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("ui.agentConfig.runPolicy")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("ui.agentConfig.runPolicy")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("ui.agentConfig.heartbeatInterval")}
              ...
              numberLabel="sec"
              numberPrefix={t("ui.agentConfig.runHeartbeatEvery")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("ui.agentConfig.runPolicy")}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("ui.agentConfig.wakeOnDemand")}
              ...
              />
              <Field label={t("ui.agentConfig.cooldown")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("ui.agentConfig.maxConcurrent")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("ui.agentConfig.continueAfterMaxTurnStop")}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("ui.agentConfig.continuationAttempts")} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("ui.agentConfig.continuationDelaySec")} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const statusLabel =
    result.status === "pass" ? translate("ui.onboarding.passed") : result.status === "warn" ? translate("ui.onboarding.warnings") : translate("ui.onboarding.failed");
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-(length:--text-micro) leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{translate("ui.onboarding.hint")}: {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
      ...
      <PopoverContent className="w-(--radix-popover-trigger-width) p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-(length:--text-nano) text-muted-foreground">{translate("ui.agentConfig.comingSoon")}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
export function ModelDropdown({
  ...
  return (
    <Field label={translate("ui.agentConfig.model")} hint={help.model}>
      <Popover
        open={open}
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value
                  || (allowDefault ? (defaultLabel ?? translate("ui.agentConfig.auto")) : required ? translate("ui.agentConfig.requiredModel") : translate("ui.agentConfig.model"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-(--radix-popover-trigger-width) p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={translate("ui.agentConfig.searchModels")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? translate("ui.agentConfig.detectModel") : detectedModel ? (detectModelLabel?.replace(/^Detect\b/, translate("ui.agentConfig.detectModel")) ?? translate("ui.agentConfig.detectModel")) : (detectModelLabel ?? translate("ui.agentConfig.detectModel"))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? translate("ui.agentConfig.refreshingModels") : translate("ui.agentConfig.refreshModels")}
            </button>
          )}
          ...
          <div className="max-h-(--sz-240px) overflow-y-auto">
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? translate("ui.agentConfig.noModelDetected"))
                    : translate("ui.agentConfig.noModels")}
                </p>
              </div>
            )}
          </div>
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ? translate("ui.agentConfig.adapterDefaultModel", { model: adapterDefaultModel })
    : translate("ui.agentConfig.cheapModelFallback");
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{translate("ui.agentConfig.cheapModel")}</div>
          <p className="text-xs text-muted-foreground">
            {translate("ui.agentConfig.cheapModelDescription")}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-(length:--text-micro) text-muted-foreground">
          {translate("ui.agentConfig.cheapModelFallsBackPrefix")} <code>{adapterDefaultModel}</code>.
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-(length:--text-micro) text-amber-500">
          {translate("ui.agentConfig.noCheapModelFallback")}
        </p>
      ) : null}
    </div>
  );
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={translate("ui.agentConfig.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? translate("ui.agentConfig.auto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-(--radix-popover-trigger-width) p-1" align="start">
