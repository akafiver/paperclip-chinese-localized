        ...
        {!rail ? (
          <>
            <Button
              ...
              aria-label={t("ui.nav.openSearch")}
              title={t("ui.nav.openSearch")}
            >
              <NavLink to="/search">
                <Search className="h-4 w-4" />
              </NavLink>
            </Button>
            <LanguageToggle variant="icon" className="text-muted-foreground shrink-0" />
            ...
            {!isMobile ? (
              peeking ? (
                <Button
                  ...
                  size="icon-sm"
                  className="text-muted-foreground shrink-0"
                  aria-label={t("ui.nav.keepSidebarExpanded")}
                  title={t("ui.nav.keepSidebarExpanded")}
                ...
                <Button
                  ...
                  aria-label={collapsed ? t("ui.nav.expandSidebar") : t("ui.nav.collapseSidebar")}
                  title={collapsed ? t("ui.nav.expandSidebar") : t("ui.nav.collapseSidebar")}
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide flex flex-col gap-4 pointer-coarse:gap-3 px-3 py-2">
        <div className="flex flex-col gap-0.5">
          ...
          {(() => {
            const newTaskButton = (
              <button
                ...
                aria-label={rail ? t("ui.nav.newTask") : undefined}
              ...
              >
                ...
                <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : "truncate"}>{t("ui.nav.newTask")}</span>
              </button>
            );
            return rail ? (
              <Tooltip>
                <TooltipTrigger asChild>{newTaskButton}</TooltipTrigger>
                <TooltipContent side="right">{t("ui.nav.newTask")}</TooltipContent>
              </Tooltip>
            ) : (
              newTaskButton
            );
          ...
          {showDecisions ? (
            <SidebarNavItem
              to="/decisions"
              label={t("ui.nav.decisions")}
              icon={ListChecks}
              badge={attentionCount}
              badgeLabel={t("ui.nav.decisions")}
            />
          ) : null}
          {conferenceRoomChatEnabled ? (
            <SidebarNavItem to="/board-chat" label={t("ui.nav.conferenceRoom")} icon={MessagesSquare} />
          ) : null}
        </div>
        ...
        <SidebarSection label={t("ui.nav.work")} collapsible={{ open: workOpen, onOpenChange: setWorkOpen }}>
          ...
          <SidebarNavItem to="/routines" label={t("ui.nav.routines")} icon={Repeat} />
          {showPipelines ? (
            <SidebarNavItem to="/pipelines" label={t("ui.nav.pipelines")} icon={GitBranch} />
          ) : null}
          {showGoalsLink ? (
            <SidebarNavItem to="/goals" label={t("ui.nav.goals")} icon={Target} />
          ) : goalsLinkPending ? (
            <div
        ...
        <SidebarSection label={t("ui.nav.company")} collapsible={{ open: companyOpen, onOpenChange: setCompanyOpen }}>
          <SidebarNavItem to="/org" label={t("ui.nav.org")} icon={Network} />
          {showApps ? <SidebarNavItem to="/apps" label={t("ui.nav.apps")} icon={AppWindow} /> : null}
          <SidebarNavItem to="/timeline" label={t("ui.nav.timeline")} icon={GanttChartSquare} />
          <SidebarNavItem to="/costs" label={t("ui.nav.costs")} icon={DollarSign} />
          <SidebarNavItem to="/activity" label={t("ui.nav.activity")} icon={History} />
          <SidebarNavItem to="/company/settings" label={t("ui.nav.settings")} icon={Settings} />
        </SidebarSection>
