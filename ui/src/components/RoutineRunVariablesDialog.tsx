    ...
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="flex h-(--sz-calc-18) max-h-(--sz-calc-18) max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-(--sz-calc-20)">
        <DialogHeader className="shrink-0 border-b border-border/60 px-6 pb-4 pr-12 pt-6">
          ...
          <DialogTitle>{t("ui.routineRun.runRoutine")}</DialogTitle>
          <DialogDescription>
            {t("ui.routineRun.description")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("ui.routineRun.agent")} *</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("ui.routineRun.agent")}
                noneLabel={t("ui.routineRun.selectAgent")}
                searchPlaceholder={t("ui.routineRun.searchAgents")}
                emptyMessage={t("ui.routineRun.noAgentsFound")}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("ui.routineRun.selectAgent")}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("ui.routineRun.project")}</Label>
              <InlineEntitySelector
                value={selection.projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("ui.routineRun.project")}
                noneLabel={t("ui.routineRun.noProject")}
                searchPlaceholder={t("ui.routineRun.searchProjects")}
                emptyMessage={t("ui.routineRun.noProjectsFound")}
                disablePortal
                openOnFocus={false}
                onChange={(projectId) => {
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("ui.routineRun.noProject")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("ui.routineRun.noValue")}</SelectItem>
                    <SelectItem value="true">{t("ui.routineRun.true")}</SelectItem>
                    <SelectItem value="false">{t("ui.routineRun.false")}</SelectItem>
                  </SelectContent>
                </Select>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("ui.routineRun.chooseValue")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("ui.routineRun.noValue")}</SelectItem>
        ...
        >
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("ui.routineRun.defaultAgentRequired")}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("ui.routineRun.missing", { fields: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("ui.routineRun.chooseWorkspaceBeforeRunning")}
            </p>
          ) : (
          ...
          >
            {isPending ? t("ui.routineRun.running") : t("ui.routineRun.runRoutine")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
