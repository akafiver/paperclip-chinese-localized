...
function getRunStatusMessage(status: string, agentName: string, elapsed: number, t: (key: string) => string): string {
  switch (status) {
    case "queued":
      return getCyclingMessage(
        [t("ui.onboardingChat.queued1"), t("ui.onboardingChat.queued2"), t("ui.onboardingChat.queued3")],
        elapsed, agentName,
      );
    case "running":
      return getCyclingMessage(
        [t("ui.onboardingChat.running1"), t("ui.onboardingChat.running2"), t("ui.onboardingChat.running3"), t("ui.onboardingChat.running4"), t("ui.onboardingChat.running5"), t("ui.onboardingChat.running6")],
        elapsed, agentName,
      );
    case "succeeded":
      return `${agentName} ${t("ui.onboardingChat.finishedSuffix")}`;
    case "failed":
      return `${agentName} ${t("ui.onboardingChat.errorSuffix")}`;
    case "cancelled":
      return `${agentName} ${t("ui.onboardingChat.cancelledSuffix")}`;
    case "timed_out":
      return `${agentName} ${t("ui.onboardingChat.timedOutSuffix")}`;
    default:
      return `${agentName} ${t("ui.onboardingChat.thinkingSuffix")}`;
  }
}
    ...
    <div className="flex flex-col h-full">
      ...
      >
        ...
        <WelcomeMessage
          ...
          onDiscuss={() => {
            setInput(t("ui.onboardingChat.discussDefault"));
            inputRef.current?.focus();
          }}
          onStart={() => sendMessage(t("ui.onboardingChat.startDefault"))}
        />
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 mb-1">
                ...
                >
                  {isAgent ? agentName : t("ui.onboardingChat.you")}
                </span>
                {isPlan && (
                  <span className="inline-flex items-center gap-0.5 text-(length:--text-nano) text-green-600 dark:text-green-400 font-medium">
                    ...
                    {t("ui.onboardingChat.planDetected")}
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center gap-2">
              {hasActiveRun ? (
                ...
                <>
                  ...
                  {getCyclingMessage([t("ui.onboardingChat.waiting1"), t("ui.onboardingChat.waiting2"), t("ui.onboardingChat.waiting3")], elapsed, agentName)}
                </>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-medium">
                  {agentName} {t("ui.onboardingChat.planReadySuffix")}
                </p>
                <p className="text-(length:--text-micro) text-muted-foreground">
                  {t("ui.onboardingChat.planReadyHint")}
                </p>
              </div>
            </div>
            <Button size="sm" onClick={onReviewPlan}>
              {t("ui.onboardingChat.reviewPlan")}
      ...
      <div className="flex items-center gap-2 border-t border-border pt-3">
        <input
          ...
          placeholder={detectedPlanCommentId ? `${t("ui.onboardingChat.revisePlaceholder")} ${agentName}...` : `${t("ui.onboardingChat.messagePlaceholder")} ${agentName}...`}
          value={input}
    ...
    <>
      ...
      {showMessage && (
        <div className="rounded-md px-3 py-2 text-sm bg-muted/50 border border-border mr-8 animate-in fade-in duration-300">
          ...
          <p>
            {t("ui.onboardingChat.welcomeHi")} <strong>{companyName}</strong>.
          </p>
          <p className="mt-1">
            {t("ui.onboardingChat.welcomeMission")}: <em>{companyGoal}</em>
          </p>
          <p className="mt-1">
            {t("ui.onboardingChat.welcomeCta")}
          </p>
        </div>
      )}
      ...
      {showChips && (
        <div className="flex gap-2 ml-auto justify-end animate-in fade-in duration-500">
          <button
            ...
            onClick={onDiscuss}
          >
            {t("ui.onboardingChat.discussFirst")}
          </button>
          <button
            ...
            onClick={onStart}
          >
            {t("ui.onboardingChat.getStarted")}
          </button>
        </div>
      )}
      ...
      {!showMessage && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground px-3 py-2">
          ...
          {phase === "waking"
            ? `${agentName} ${t("ui.onboardingChat.wakingUpSuffix")}`
            : `${agentName} ${t("ui.onboardingChat.composingSuffix")}`}
        </div>
      )}
    </>
  );
