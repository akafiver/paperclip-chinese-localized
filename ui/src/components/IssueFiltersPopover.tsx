  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? t("ui.issueFilters.filtersCount", { count: activeFilterCount }) : t("ui.issueFilters.filter")) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? t("ui.issueFilters.filtersCount", { count: activeFilterCount }) : t("ui.issueFilters.filter")}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("ui.issueFilters.filters")}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
              ...
              >
                {t("ui.issueFilters.clear")}
              </button>
            ) : null}
          </div>

          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("ui.issueFilters.quickFilters")}</span>
            <div className="flex flex-wrap gap-1.5">
              {issueQuickFilterPresets.map((preset) => {
                ...
                return (
                  ...
                  >
                    {t(`ui.issueFilters.quick.${preset.label.toLowerCase()}`)}
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("ui.issueFilters.status")}</span>
                <div className="space-y-0.5">
                  {issueStatusOrder.map((status) => (
                    <label key={status} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{t(`ui.issueFilters.statuses.${status}`)}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("ui.issueFilters.priority")}</span>
                <div className="space-y-0.5">
                  {issuePriorityOrder.map((priority) => (
                    <label key={priority} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{t(`ui.issueFilters.priorities.${priority}`)}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("ui.issueFilters.responsible")}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("ui.issueFilters.noResponsible")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("ui.issueFilters.me")}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("ui.issueFilters.creator")}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("ui.issueFilters.removeCreator", { label: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("ui.issueFilters.searchCreators")}
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("ui.issueFilters.noCreatorsMatch")}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("ui.issueFilters.project")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("ui.issueFilters.labels")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {labels.map((label) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("ui.issueFilters.workspace")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
              ...
              {enableExternalObjectFilters ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("ui.issueFilters.externalObjectStatus")}</span>
                  <div className="space-y-0.5">
                    {externalObjectFilterOrder.map((value) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-sm">{t(`ui.issueFilters.externalObjectStatuses.${value}`)}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("ui.issueFilters.visibility")}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("ui.issueFilters.liveRunsOnly")}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("ui.issueFilters.hideRoutineRuns")}</span>
                  </label>
                ) : null}
              </div>
            </div>
