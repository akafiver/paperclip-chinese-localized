...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused ? t("ui.sidebarAgents.resumeAgent") : t("ui.sidebarAgents.pauseAgent");
  ...
  const pauseResumeDisabledLabel = disabled
    ? t("ui.sidebarAgents.updating")
    : isBudgetPaused
      ? t("ui.sidebarAgents.budgetPaused")
      : isPaused && hasInvalidOrgChain
        ? t("ui.sidebarAgents.invalidOrgChain")
      : pauseResumeLabel;
  const trailingLabel = [
    builtInStatus ? t("ui.sidebarAgents.builtInAgentStatus", { status: builtInStatus.replace(/_/g, " ") }) : null,
    hasInvalidOrgChain ? t("ui.sidebarAgents.invalidReportingChain") : null,
  ...
  const navItem = (
    <SidebarNavItem
      ...
      trailing={
        builtInStatus || hasInvalidOrgChain ? (
          <span className="ml-1 flex shrink-0 items-center gap-1">
            ...
            {hasInvalidOrgChain ? (
              <AlertTriangle className="h-3.5 w-3.5 shrink-0 text-amber-500" aria-label={t("ui.sidebarAgents.invalidReportingChain")} />
            ) : null}
          </span>
        ) : undefined
      }
      ...
      liveAccessory={
        agent.pauseReason === "budget" ? <BudgetSidebarMarker title={t("ui.sidebarAgents.agentPausedByBudget")} /> : undefined
      }
    />
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("ui.sidebarAgents.openActionsFor", { agentName: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-48">
          {onToggleStar ? (
            <>
              ...
              >
                ...
                <span>{starred ? t("ui.sidebarAgents.removeFromStarred") : t("ui.sidebarAgents.starAgent")}</span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          ) : null}
          <DropdownMenuItem asChild>
            ...
            >
              ...
              <span>{t("ui.sidebarAgents.editAgent")}</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("ui.sidebarAgents.agentPausedByBudgetLimits") : undefined}
          >
          ...
          >
            ...
            <span>{leaving ? t("ui.sidebarAgents.leaving") : t("ui.sidebarAgents.leaveAgent")}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
...
export function SidebarAgents({ streamlined = false }: { streamlined?: boolean } = {}) {
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: action === "pause" ? t("ui.sidebarAgents.agentPaused") : t("ui.sidebarAgents.agentResumed"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause" ? t("ui.sidebarAgents.couldNotPauseAgent") : t("ui.sidebarAgents.couldNotResumeAgent"),
  ...
  const agentSortChoices: SidebarSectionRadioChoice[] = useMemo(() => [
    { value: "top", label: t("ui.sidebarAgents.sortTop") },
    { value: "alphabetical", label: t("ui.sidebarAgents.sortAlphabetical") },
    { value: "recent", label: t("ui.sidebarAgents.sortRecent") },
  ], [t]);

  return (
    <SidebarSection
      label={t("ui.sidebarAgents.sectionLabel")}
      collapsible={{ open, onOpenChange: setOpen }}
      headerAction={{
        ariaLabel: t("ui.sidebarAgents.newAgent"),
        icon: Plus,
        onClick: openNewAgent,
      }}
      menu={{
        ariaLabel: t("ui.sidebarAgents.sectionActions"),
        actions: [
          { type: "item", label: t("ui.sidebarAgents.browseAgents"), icon: Users, href: "/agents/all" },
          { type: "separator" },
        ],
        radioLabel: t("ui.sidebarAgents.agentSort"),
      ...
      {showSeeAllLink && (() => {
        ...
        const seeAllLink = (
          <Link
            ...
            aria-label={rail ? t("ui.sidebarAgents.seeAllAgents") : undefined}
            onClick={() => {
              if (isMobile) setSidebarOpen(false);
            }}
          ...
          >
            ...
            <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : undefined}>{t("ui.sidebarAgents.seeAllAgents")}</span>
          </Link>
        );
        return rail ? (
          <Tooltip>
            <TooltipTrigger asChild>{seeAllLink}</TooltipTrigger>
            <TooltipContent side="right">{t("ui.sidebarAgents.seeAllAgents")}</TooltipContent>
          </Tooltip>
        ) : (
          seeAllLink
        );
