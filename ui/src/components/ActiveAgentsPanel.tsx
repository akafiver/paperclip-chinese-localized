...
function RunCardRecoveryChip({ action }: { action: IssueRecoveryAction }) {
  ...
  return (
    <Badge variant="outline"
      ...
      title={t("ui.activeAgentsPanel.recoveryTitle", { label: tone.label })}
      className={cn(
...
export function ActiveAgentsPanel({
  ...
  const { t } = useTranslation();
  const panelTitle = title ?? t("ui.activeAgentsPanel.title");
  const resolvedEmptyMessage = emptyMessage ?? t("ui.activeAgentsPanel.emptyMessage");
      ...
      {showMoreLink && hiddenRunCount > 0 && (
        <div className="mt-3 flex justify-end text-xs text-muted-foreground">
          <Link to="/dashboard/live" className="hover:text-foreground hover:underline">
            {t("ui.activeAgentsPanel.moreRuns", { count: hiddenRunCount })}
          </Link>
        </div>
      )}
    </div>
...
const AgentRunCard = memo(function AgentRunCard({
  ...
  const { t } = useTranslation();
  const runTimingLabel = isActive
    ? t("ui.activeAgentsPanel.liveNow")
    : run.finishedAt
      ? t("ui.activeAgentsPanel.finishedAt", { time: relativeTime(run.finishedAt, t) })
      : t("ui.activeAgentsPanel.startedAt", { time: relativeTime(run.createdAt, t) });
