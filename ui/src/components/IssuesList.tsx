...
type IssuesListTranslate = (key: string, params?: Record<string, unknown>) => string;

function issueStatusLabel(status: IssueStatus, t: IssuesListTranslate): string {
  return t(`ui.issuesList.status.${status}`);
}
  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        placeholder={t("ui.issuesList.searchTasks")}
        className="pl-7 text-xs sm:text-sm"
        aria-label={t("ui.issuesList.searchTasksAria")}
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {t("ui.issuesList.subtasksDone", { done: summary.doneCount, total: summary.totalCount })}
            </span>
            <span className="text-muted-foreground">
              {t("ui.issuesList.subtasksInProgress", { count: summary.inProgressCount })}
            </span>
            <span className="text-muted-foreground">
              {t("ui.issuesList.subtasksBlocked", { count: summary.blockedCount })}
            </span>
            {showCostSummary && (
              <>
                <span
                  className="text-muted-foreground tabular-nums"
                  title={t("ui.issuesList.costSummaryTitle", {
                    runCount: costSummary.runCount.toLocaleString(),
                    issueCount: costSummary.issueCount,
                  })}
                >
                  {t("ui.issuesList.tokens", { count: formatTokens(totalTokens) })}
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {t("ui.issuesList.runtime", { duration: formatDurationMs(costSummary.runtimeMs) })}
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label={t("ui.issuesList.subtasksProgress")}
            aria-valuemin={0}
          ...
          >
            {statusEntries.map(({ status, count }) => (
              <span
                ...
                title={t("ui.issuesList.statusCount", { status: issueStatusLabel(status, t), count })}
                aria-hidden="true"
              />
            ))}
          </div>
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next" ? t("ui.issuesList.nextUp") : t("ui.issuesList.waitingOnBlockers")}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">{t("ui.issuesList.noActiveSubtasks")}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">{t("ui.issuesList.allSubtasksDone")}</div>
          ) : (
            <div className="text-sm font-medium text-foreground">{t("ui.issuesList.noActionableSubtasks")}</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
  ...
  const creatorOptions = useMemo<CreatorOption[]>(() => {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board" ? t("ui.issuesList.boardFallback") : t("ui.issuesList.me"),
        kind: "user",
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "workspace") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_workspace" ? t("ui.issuesList.noWorkspace") : (workspaceNameMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "project") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_project" ? t("ui.issuesList.noProject") : (projectById.get(key)?.name ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? t("ui.issuesList.noParent") : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
    ...
    return Object.keys(groups).map((key) => ({
      ...
      label:
        key === "__unassigned"
          ? t("ui.issuesList.unassigned")
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId, companyUserLabelMap) ?? t("ui.issuesList.userFallback"))
  ...
  const createActionLabel = createIssueLabel ? t("ui.issuesList.createCustom", { label: createIssueLabel }) : t("ui.issuesList.createTask");
  const createButtonLabel = createIssueLabel ? t("ui.issuesList.newCustom", { label: createIssueLabel }) : t("ui.issuesList.newTask");
    ...
    <div ref={rootRef} className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          {/* View mode toggle */}
          <div className="flex items-center border border-border rounded-md overflow-hidden mr-1" role="group" aria-label={t("ui.issuesList.viewMode")}>
            <button
              ...
              title={t("ui.issuesList.listView")}
              aria-label={t("ui.issuesList.listView")}
            ...
            <button
              ...
              title={t("ui.issuesList.boardView")}
              aria-label={t("ui.issuesList.boardView")}
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? t("ui.issuesList.disableNesting") : t("ui.issuesList.enableNesting")}
            >
          ...
          {viewState.viewMode === "board" && (
            <>
              <Button
                ...
                title={boardCompactCards ? t("ui.issuesList.useComfortableCards") : t("ui.issuesList.useCompactCards")}
              >
              ...
              <Button
                ...
                title={boardCollapsedStatuses.length > 0 ? t("ui.issuesList.expandColdLanes") : t("ui.issuesList.collapseColdLanes")}
              >
              ...
              <Button
                ...
                title={t("ui.issuesList.resetBoardDensity")}
              >
          ...
          <IssueColumnPicker
            ...
            title={t("ui.issuesList.chooseColumns")}
            iconOnly
          />
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("ui.issuesList.sort")}>
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["workflow", t("ui.issuesList.sortFields.workflow")],
                    ["status", t("ui.issuesList.sortFields.status")],
                    ["priority", t("ui.issuesList.sortFields.priority")],
                    ["title", t("ui.issuesList.sortFields.title")],
                    ["created", t("ui.issuesList.sortFields.created")],
                    ["updated", t("ui.issuesList.sortFields.updated")],
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("ui.issuesList.group")}>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("ui.issuesList.groupFields.status")],
                    ["priority", t("ui.issuesList.groupFields.priority")],
                    ["assignee", t("ui.issuesList.groupFields.assignee")],
                    ["project", t("ui.issuesList.groupFields.project")],
                    ["workspace", t("ui.issuesList.groupFields.workspace")],
                    ["parent", t("ui.issuesList.groupFields.parent")],
                    ["none", t("ui.issuesList.groupFields.none")],
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          {t("ui.issuesList.searchLimit", { count: ISSUE_SEARCH_RESULT_LIMIT })}
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          {t("ui.issuesList.boardColumnLimit", { count: ISSUE_BOARD_COLUMN_RESULT_LIMIT })}
        </p>
      )}
      {!isLoading && !externalObjectFilterLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("ui.issuesList.noTasksMatch")}
        ...
        <>
                          ...
                          <>
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <Badge variant="outline"
                                ...
                                aria-label={t("ui.issuesList.needsNextStep")}
                                title={t("ui.issuesList.needsNextStepTitle")}
                              >
                                <CircleDot className="h-3 w-3" />
                                {t("ui.issuesList.needsNextStep")}
                              </Badge>
                            ) : null}
                          </>
                        )}
                                ...
                                >
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                        ...
                                        <Identity
                                          name={assigneeUserLabel ?? t("ui.issuesList.userFallback")}
                                  ...
                                  >
                                    <input
                                      ...
                                      placeholder={t("ui.issuesList.searchResponsible")}
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? t("ui.issuesList.loadingMoreTasks")
                  : remainingIssueRowCount > 0
                    ? t("ui.issuesList.renderingTasks", { rendered: Math.min(renderedIssueRowLimit, filtered.length), total: filtered.length })
                    : t("ui.issuesList.scrollToLoadMore")}
              </p>
            </div>
          )}
        </>
