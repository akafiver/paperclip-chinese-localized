  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("ui.agentProperties.status")}>
          <AgentStatusBadge status={agent.status} />
        </PropertyRow>
        {lastErrorIsActive && agent.errorReason && (
          <PropertyRow label={t("ui.agentProperties.errorReason")}>
            <span className="text-xs text-red-600 dark:text-red-400 break-words min-w-0">
              {agent.errorReason}
            </span>
          </PropertyRow>
        )}
        <PropertyRow label={t("ui.agentProperties.role")}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t("ui.agentProperties.title")}>
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("ui.agentProperties.adapter")}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("ui.agentProperties.session")}>
        ...
        {runtimeState?.lastError && (
          <PropertyRow label={lastErrorIsActive ? t("ui.agentProperties.lastError") : t("ui.agentProperties.lastRunError")}>
            <span
              className={
                lastErrorIsActive
        ...
        {agent.lastHeartbeatAt && (
          <PropertyRow label={t("ui.agentProperties.lastHeartbeat")}>
            <span className="text-sm">{formatDate(agent.lastHeartbeatAt)}</span>
          </PropertyRow>
        )}
        {agent.reportsTo && (
          <PropertyRow label={t("ui.agentProperties.reportsTo")}>
            {reportsToAgent ? (
        ...
        <PropertyRow label={t("ui.agentProperties.created")}>
