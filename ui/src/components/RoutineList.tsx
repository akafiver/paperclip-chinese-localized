...
export function RoutineListRow<TRoutine extends RoutineListRowItem>({
  ...
  const runLabel = runningRoutineId === routine.id ? t("ui.routines.running") : t("ui.routines.runNow");
  const statusLabel = isArchived
    ? t("ui.routines.status.archived")
    : isDraft
      ? t("ui.routines.status.draft")
      : routine.status === "paused"
        ? t("ui.routines.status.paused")
        : null;
  const toggleStateLabel = isArchived
    ? t("ui.routines.status.archived")
    : isDraft
      ? t("ui.routines.status.draft")
      : enabled
        ? t("ui.routines.status.on")
        : t("ui.routines.status.off");
      ...
      {selectMode ? (
        ...
        >
          <input
            ...
            checked={selected}
            aria-label={t("ui.routines.selectRoutine", { title: routine.title })}
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span className="flex items-center gap-2">
            ...
            />
            <span>{routine.projectId ? (project?.name ?? t("ui.routines.unknownProject")) : t("ui.routines.noProjectLabel")}</span>
          </span>
          <span className="flex items-center gap-2">
            {agent?.icon ? <AgentIcon icon={agent.icon} className="h-3.5 w-3.5 shrink-0" /> : null}
            <span>{routine.assigneeAgentId ? (agent?.name ?? t("ui.routines.unknownAgent")) : t("ui.routines.noDefaultAgent")}</span>
          </span>
          <span>
            {routine.lastRun?.triggeredAt ? formatLastRunTimestamp(routine.lastRun.triggeredAt) : t("ui.routines.never")}
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        ...
        <div className="flex items-center gap-3">
          <ToggleSwitch
            ...
            aria-label={enabled ? t("ui.routines.disableRoutine", { title: routine.title }) : t("ui.routines.enableRoutine", { title: routine.title })}
          />
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("ui.routines.moreActionsFor", { title: routine.title })}>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem
              ...
              disabled={isStatusPending || isArchived || disableToggle}
            >
              {enabled ? t("ui.routines.pause") : t("ui.routines.enable")}
            </DropdownMenuItem>
            {!hideArchiveAction && onToggleArchived ? (
              ...
              >
                {routine.status === "archived" ? t("ui.routines.restore") : t("ui.common.archive")}
              </DropdownMenuItem>
            ) : null}
          </DropdownMenuContent>
        </DropdownMenu>
