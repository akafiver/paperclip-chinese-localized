...
import { hasProcessMetadata, isPidAlive, isProcessGroupAlive, terminateLocalService } from "../local-service-supervisor.js";
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function cleanupSourceResolvedRunProcess(input: {
    ...
    if (!hasProcessMetadata(input.run.processPid, input.run.processGroupId) && !runningProcesses.has(input.run.id)) {
      return {
        attempted: false,
  ...
  function buildStrandedIssueRecoveryDescription(input: {
    ...
    if (input.recoveryCause === SUCCESSFUL_RUN_MISSING_STATE_REASON) {
      ...
      return [
        "Paperclip 已停止自动继续，因为一次成功运行之后任务仍缺少最终处置。",
        "",
        "这不是 runtime 或 adapter 崩溃。源运行已经成功；剩余问题是任务没有被明确标记为 `done`、`in_review`、`blocked`、委派后续任务，或记录明确继续路径。为避免重复执行外部动作，Paperclip 不会自动再次运行 Agent。",
        "",
        "## 安全证据 / Safe Evidence",
        "",
        `- Source issue: ${sourceIssue}`,
        `- Source run: ${sourceRunLink}`,
        `- Latest follow-up run: ${runLink}`,
        ...
        "- Suggested action: choose and record exactly one final outcome: complete, continue, retry, blocked, cancelled, or assign an explicit reviewer/follow-up owner.",
        "",
        "## Required Action",
        "",
