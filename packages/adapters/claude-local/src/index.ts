...
- filesystemScope (string, optional): defaults to "workspace" for local CLI runs, confining filesystem access to the workspace plus managed Claude config. Configure filesystemExtraPaths for intentional outside access.
...
- filesystemSandboxCommand (string, optional): sandbox executable name or absolute path; defaults to "sandbox-exec" on macOS and "bwrap" elsewhere.
- dangerouslyDisableFilesystemSandbox (boolean, optional): disables the default workspace filesystem sandbox for local CLI runs. Intended only for trusted maintenance/debugging.
...
Notes:
- filesystemScope and networkScope are spawn-level confinement and are orthogonal to Claude permission flags. Filesystem confinement uses sandbox-exec on macOS and Bubblewrap on Linux; network allowlist is Linux-only. Both select the CLI engine in auto mode; engine="acp" is rejected because ACP confinement is not yet supported. networkScope="allowlist" injects HTTP_PROXY/HTTPS_PROXY for the CLI while its private network namespace blocks direct sockets, so every required provider/API hostname must be listed explicitly.
